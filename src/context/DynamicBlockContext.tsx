...
/* ---------------- Context ---------------- */
export const DynamicBlockContext = createContext<DynamicBlockContextType | undefined>(undefined);

/* ---------------- Provider ---------------- */
export const DynamicBlockProvider: React.FC<DynamicBlockProviderProps> = ({ children }) => {
  ...
  const [dynamicData, setDynamicData] = useState<Map<number, any>>(() => {
    try {
      const ssrData = typeof window !== 'undefined' ? window.__DYNAMIC_BLOCK_DATA__ : undefined;
      if (!ssrData || typeof ssrData !== 'object' || Array.isArray(ssrData)) return new Map();
  ...
  const [errorBlocks, setErrorBlocks] = useState<Map<number, string>>(new Map());

  const registerDynamicBlock = useCallback((blockId: number, _dataSource: string) => {
  ...
  const value = useMemo<DynamicBlockContextType>(
    ...
    [dynamicData, loadingBlocks, errorBlocks, registerDynamicBlock, refreshBlock, isBlockDynamic]
  );

  return <DynamicBlockContext.Provider value={value}>{children}</DynamicBlockContext.Provider>;
};
