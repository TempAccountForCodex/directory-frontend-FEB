...
const makeField = (conditional?: FieldDefinition['conditional']): FieldDefinition => ({
  name: 'testField',
  type: 'text',
  label: 'Test Field',
  conditional,
...
describe('shouldShowField — equals', () => {
  it('returns true when values[field] === expected', () => {
    const field = makeField({ field: 'theme', operator: 'equals', value: 'dark' });
...
describe('shouldShowField — notEquals', () => {
  it('returns true when values[field] !== expected', () => {
    const field = makeField({ field: 'plan', operator: 'notEquals', value: 'free' });
...
describe('shouldShowField — isNotEmpty', () => {
  it('returns true when value is a non-empty string', () => {
    ...
    expect(shouldShowField(field, { website: 'https://example.com' })).toBe(true);
  });
...
describe('shouldShowField — contains', () => {
  it('returns true when array target includes expected value', () => {
    const field = makeField({ field: 'tags', operator: 'contains', value: 'tech' });
  ...
  it('returns false when array target does not include expected value', () => {
    const field = makeField({ field: 'tags', operator: 'contains', value: 'finance' });
    expect(shouldShowField(field, { tags: ['tech', 'startup'] })).toBe(false);
  });

  it('returns true when string target includes expected substring', () => {
    const field = makeField({ field: 'description', operator: 'contains', value: 'hello' });
    expect(shouldShowField(field, { description: 'say hello world' })).toBe(true);
  });

  it('returns false when string target does not include expected substring', () => {
    const field = makeField({ field: 'description', operator: 'contains', value: 'goodbye' });
    expect(shouldShowField(field, { description: 'say hello world' })).toBe(false);
  });

  it('handles undefined target gracefully (no crash)', () => {
    const field = makeField({ field: 'missing', operator: 'contains', value: 'x' });
...
describe('shouldShowField — greaterThan', () => {
  it('returns true when numeric target > expected', () => {
    const field = makeField({ field: 'age', operator: 'greaterThan', value: 18 });
...
describe('shouldShowField — lessThan', () => {
  it('returns true when numeric target < expected', () => {
    const field = makeField({ field: 'score', operator: 'lessThan', value: 50 });
...
describe('shouldShowField — flat path access', () => {
  it('accesses values["backgroundType"] not values["background"]["backgroundType"]', () => {
    const field = makeField({ field: 'backgroundType', operator: 'equals', value: 'image' });
    // Correct flat key
    ...
    expect(shouldShowField(field, { background: { backgroundType: 'image' } })).toBe(false);
  });
});
