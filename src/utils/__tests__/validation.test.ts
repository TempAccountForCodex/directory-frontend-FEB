...
describe('validateField', () => {
  ...
  it('maxLength: pushes error when string is too long', () => {
    expect(validateField('abcdef', { maxLength: 5 })).toContain('Max 5 characters');
  });
  ...
  it('pattern: pushes error when value does not match', () => {
    expect(validateField('hello world', { pattern: /^\w+$/ })).toContain('Invalid format');
  });
  ...
  it('email: pushes error for invalid email (no @)', () => {
    expect(validateField('notanemail', { email: true })).toContain('Invalid email address');
  });

  it('email: pushes error for invalid email (no domain)', () => {
    expect(validateField('user@', { email: true })).toContain('Invalid email address');
  });
  ...
  it('custom: pushes returned string when validator returns an error', () => {
    const rules: ValidationRules = {
      custom: (v) => (String(v).includes('bad') ? 'Contains forbidden word' : null),
    };
    expect(validateField('this is bad', rules)).toContain('Contains forbidden word');
  });
