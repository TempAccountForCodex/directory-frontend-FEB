...
import { FieldType, ConditionalOperator } from '../../components/DynamicFields/types';
...
describe('buildDependencyGraph', () => {
  ...
  it('builds graph with multiple dependencies', () => {
    const fields = [makeField('a'), makeField('b', 'a'), makeField('c', 'a'), makeField('d', 'b')];
    const graph = buildDependencyGraph(fields);
...
describe('Performance', () => {
  it('processes 50 fields in under 5ms', () => {
    ...
    for (let i = 0; i < 50; i++) {
      fields.push(makeField(`field_${i}`, i > 0 ? `field_${i - 1}` : undefined));
    }
