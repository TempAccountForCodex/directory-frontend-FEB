...
describe('useDynamicBlockData', () => {
  ...
  it('returns correct shape: { data, loading, error, refresh, lastUpdated }', () => {
    const { result } = renderHook(() => useDynamicBlockData(1, 'TEXT', null), { wrapper });
  ...
  it('does not fetch when enabled is false', () => {
    renderHook(() => useDynamicBlockData(1, 'TEXT', '/api/data', { enabled: false }), { wrapper });
    expect(mockFetch).not.toHaveBeenCalled();
  });

  it('returns initialData when dataSource is null and initialData provided', () => {
    const initialData = { foo: 'bar' };
    const { result } = renderHook(() => useDynamicBlockData(1, 'TEXT', null, { initialData }), {
      wrapper,
    });
  ...
  it('fetches data when dataSource is provided (debounceMs: 0)', async () => {
    renderHook(() => useDynamicBlockData(1, 'TEXT', '/api/blocks/1/data', { debounceMs: 0 }), {
      wrapper,
    });
  ...
  it('does not fetch when refreshInterval is 0 and no manual refresh', () => {
    ...
    renderHook(() => useDynamicBlockData(1, 'TEXT', null, { refreshInterval: 0 }), { wrapper });
    expect(mockFetch).not.toHaveBeenCalled();
  });
  ...
  it('accepts onError option without throwing', async () => {
    ...
    const { result } = renderHook(() => useDynamicBlockData(1, 'TEXT', null, { onError }), {
      wrapper,
    });
