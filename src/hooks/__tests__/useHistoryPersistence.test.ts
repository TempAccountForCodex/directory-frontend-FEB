...
function makeSnapshot(state: string, description: string, timestamp = Date.now()) {
...
describe('useHistory — sessionStorage persistence (9.8.2)', () => {
  ...
  it('persists history to sessionStorage after push', () => {
    ...
    expect(global.sessionStorage.setItem).toHaveBeenCalledWith(SESSION_KEY, expect.any(String));
  });
  ...
  it('uses sessionStorage key "undo_redo_history"', () => {
    ...
    const calls = (global.sessionStorage.setItem as ReturnType<typeof vi.fn>).mock.calls;
  ...
  it('loads history from sessionStorage on mount', () => {
    ...
    import('../../utils/historySerializer').then(({ compressHistory }) => {
      const preExisting = [makeSnapshot('restored-state', 'Restored action', 1700000000000)];
      const compressed = compressHistory(preExisting);
      (global.sessionStorage.getItem as ReturnType<typeof vi.fn>).mockReturnValue(compressed);
    });
  });
  ...
  it('persists (removes) sessionStorage after clear', () => {
    ...
    const setCalls = (global.sessionStorage.setItem as ReturnType<typeof vi.fn>).mock.calls;
    const removeCalls = (global.sessionStorage.removeItem as ReturnType<typeof vi.fn>).mock.calls;
    const touched =
...
describe('useHistory — QuotaExceededError handling (9.8.3)', () => {
  ...
  it('does not throw when sessionStorage.setItem throws QuotaExceededError', () => {
    (global.sessionStorage.getItem as ReturnType<typeof vi.fn>).mockReturnValue(null);
    (global.sessionStorage.setItem as ReturnType<typeof vi.fn>).mockImplementation(() => {
