...
describe('readBackup', () => {
  ...
  it('returns null and removes stale entry (>24h)', () => {
    const staleEntry = { data: { blocks: [] }, timestamp: Date.now() - 25 * 60 * 60 * 1000 };
...
describe('useLocalStorageBackup', () => {
  ...
  it('detects existing backup on mount', () => {
    const entry = { data: { blocks: [{ id: '99' }] }, timestamp: Date.now() - 5000 };
  ...
  it('restoreBackup returns data and clears state', () => {
    const entry = { data: { blocks: [{ id: 'a' }] }, timestamp: Date.now() - 5000 };
    localStore['UNSAVED_CHANGES_42_7'] = JSON.stringify(entry);
  ...
  it('clearBackup removes localStorage entry', () => {
    localStore['UNSAVED_CHANGES_42_7'] = JSON.stringify({ data: {}, timestamp: Date.now() });
