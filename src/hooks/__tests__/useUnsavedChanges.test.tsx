...
vi.mock('react-router-dom', async () => {
  ...
  return {
    ...actual,
    useBlocker: (cb: (args: { currentLocation: any; nextLocation: any }) => boolean) => {
...
describe('useUnsavedChanges', () => {
  ...
  describe('initial state and shape', () => {
    it('returns expected shape on mount', () => {
      const { result } = renderHook(() => useUnsavedChanges({ hasUnsavedChanges: false }), {
  ...
  describe('beforeunload listener', () => {
    it('adds beforeunload listener when hasUnsavedChanges=true (standalone)', () => {
      ...
      renderHook(() => useUnsavedChanges({ hasUnsavedChanges: true }), { wrapper: RouterWrapper });
    ...
    it('does NOT add beforeunload when skipBeforeUnload=true (useAutosave mode)', () => {
      ...
      renderHook(() => useUnsavedChanges({ hasUnsavedChanges: true, skipBeforeUnload: true }), {
        wrapper: RouterWrapper,
      });
    ...
    it('removes beforeunload listener when hasUnsavedChanges becomes false', () => {
      ...
      const { rerender } = renderHook(
        ({ dirty }: { dirty: boolean }) => useUnsavedChanges({ hasUnsavedChanges: dirty }),
    ...
    it('removes beforeunload listener on unmount', () => {
      ...
      const { unmount } = renderHook(() => useUnsavedChanges({ hasUnsavedChanges: true }), {
        wrapper: RouterWrapper,
      });
  ...
  describe('useBlocker integration', () => {
    ...
    it('blocker callback returns false when no unsaved changes', () => {
      renderHook(() => useUnsavedChanges({ hasUnsavedChanges: false }), { wrapper: RouterWrapper });
    ...
    it('showDialog is true when blocker.state is blocked', () => {
      ...
      const { result } = renderHook(() => useUnsavedChanges({ hasUnsavedChanges: true }), {
        wrapper: RouterWrapper,
      });
