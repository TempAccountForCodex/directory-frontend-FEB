...
beforeEach(() => {
  ...
  (sessionStorage.removeItem as ReturnType<typeof vi.fn>).mockImplementation((key: string) => {
    delete store[key];
  });
...
describe('useAIQuestionnaire', () => {
  ...
  it('updateSocialLink updates a specific social platform', () => {
    ...
    expect(result.current.data.socialLinks.facebook).toBe('https://fb.com/test');
  ...
  describe('validateRequired', () => {
    ...
    it('passes with all valid required fields', () => {
      ...
      act(() => {
        ...
        result.current.updateField('services', 'We offer great food and dining experiences');
      });
      let validation: any;
      act(() => {
        validation = result.current.validateRequired();
  ...
  describe('sessionStorage persistence', () => {
    it('persists state to sessionStorage on field update', async () => {
      const { result, rerender } = renderHook(() => useAIQuestionnaire('tpl-42'));
      act(() => {
    ...
    it('reset clears state and sessionStorage', () => {
      const { result, rerender } = renderHook(() => useAIQuestionnaire('tpl-50'));
      act(() => {
