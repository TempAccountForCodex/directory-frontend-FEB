...
function wrapper({ children }: { children: React.ReactNode }) {
  return React.createElement(MemoryRouter, { initialEntries: ['/directory'] }, children);
}

function wrapperWithSearch(search: string) {
  return function WrapperWithSearch({ children }: { children: React.ReactNode }) {
    return React.createElement(MemoryRouter, { initialEntries: [`/directory${search}`] }, children);
  };
}
...
describe('useDirectorySearch', () => {
  ...
  it('activeFilters array contains chip info for each active filter', () => {
    ...
    expect(result.current.activeFilters.some((f) => f.key === 'category')).toBe(true);
    expect(result.current.activeFilters.some((f) => f.key === 'hasReviews')).toBe(true);

    // Each active filter has onRemove callback
    const catFilter = result.current.activeFilters.find((f) => f.key === 'category')!;
  ...
  it('loading is true during fetch and false when complete', async () => {
    ...
    fetchMock.mockImplementation((url: string) => {
      if (url.includes('/api/directory/meta')) {
        return Promise.resolve({ ok: true, json: () => Promise.resolve(mockMetaResponse) });
      }
    ...
    // resolve the fetch
    await act(async () => {
      resolveFetch!({ ok: true, json: () => Promise.resolve(mockListingsResponse) });
    });
