import type { BusinessData, BlogPost, Feature, Review } from '../landingTemplates/types/BusinessData';
...
const getBlockTypeKey = (block: TemplateEditorBlock | RawTemplateBlock | null | undefined): string => {
  if (!block) {
    return '';
  }
...
const inferFrontendTemplateIdFromBlocks = (
  ...
  if (typeof heroStyle === 'string' && heroStyle.trim().toLowerCase() === 'executive') {
      ...
      sections: [
        ...
        },
        {
          ...
          buildContent: () => ({
            ...
            description: 'Add your email and receive editorial updates directly in your inbox.',
      ...
      sections: [
        {
          ...
          blockType: 'HERO',
          buildContent: (data) => ({
            heading: data.tagline || 'Beautiful presentation for modern brands.',
            subheading: data.description || '',
        ...
        },
        {
          ...
          blockType: 'TEXT',
          buildContent: () => ({
            title: 'Direction for launches, events, and polished company moments.',
            body:
              'Use this section for campaigns, seasonal messaging, private appointments, or company capabilities.',
          }),
        },
        {
          key: 'services',
  ...
  'company-executive': [
    {
      ...
      sections: [
        ...
        },
        {
          ...
          blockType: 'TEXT',
          buildContent: (data) => ({
            title: 'Driving innovation and excellence for corporate success worldwide.',
            body: data.description || data.tagline || '',
          }),
        },
        {
          ...
          buildContent: (data) => ({
            ...
            features:
              featuresToFeatureItems(data.features || []).length
        ...
        },
        {
          ...
          buildContent: () => ({
            ...
            subheading: 'A simple executive flow built to move from strategy to launch with clarity.',
...
const normalizePersistedPages = (pages: unknown[] | null | undefined): TemplateEditorPage[] => {
  ...
  return pages.map((page, pageIndex) => {
    ...
    return {
      ...
      blocks: rawBlocks.map((block, blockIndex) => {
        ...
        return {
          id: String(typedBlock.id ?? `${typedPage.id ?? pageIndex}-block-${blockIndex}`),
...
const getTemplateSectionKeys = (templateId: string): string[] =>
  (TEMPLATE_PAGE_SCHEMAS[templateId] || []).flatMap((page) => page.sections.map((section) => section.key));

const getAllBlocks = (pages: TemplateEditorPage[]): TemplateEditorBlock[] =>
  pages.flatMap((page) => (Array.isArray(page.blocks) ? page.blocks : []));

const getOrderedBlocksForHomePage = (pages: TemplateEditorPage[]): TemplateEditorBlock[] => {
  const homePage =
...
const getOrderedSectionContentMap = (
  ...
  const templateSlots = new Set(
    (TEMPLATE_PAGE_SCHEMAS[templateId] || [])
      .flatMap((page) => page.sections.map((section) => section.key))
  );

  orderedBlocks.forEach((block) => {
    const sectionKey = typeof block.content?.editorSection === 'string'
      ? block.content.editorSection.trim()
      : getCompatibleSectionKey(templateId, block);

    if (!sectionKey || !templateSlots.has(sectionKey) || orderedContentMap.has(sectionKey)) {
      return;
    }
...
const getTemplateSectionMap = (templateId: string, pages: TemplateEditorPage[]) => {
...
const mergeTemplateBlockContent = (
  ...
  if (persistedTheme && typeof persistedTheme === 'object' && !Array.isArray(persistedTheme)) {
    return {
      ...seededContent,
      ...persistedContent,
      [TEMPLATE_THEME_CONTENT_KEY]: {
        ...(seededContent[TEMPLATE_THEME_CONTENT_KEY] as Record<string, unknown> | undefined),
...
const hydrateSeededPages = (
  ...
  const persistedPageMap = new Map(
    persistedPages.map((page) => {
      const pageKey = page.isHome ? '__home__' : page.path;
      return [pageKey, page];
    })
  );
...
const getThemeSettingsFromUnknown = (value: unknown): TemplateThemeSettings | null => {
  ...
  if (typeof source.secondaryColor === 'string' && source.secondaryColor.trim()) {
...
export const readTemplateThemeSettingsFromPages = (
  ...
  for (const block of getAllBlocks(pages)) {
    const themeSettings = getThemeSettingsFromUnknown(block.content?.[TEMPLATE_THEME_CONTENT_KEY]);
    if (themeSettings) {
    ...
    index === 0
      ? {
          ...
          content: {
            ...block.content,
            [TEMPLATE_THEME_CONTENT_KEY]: {
              ...(block.content?.[TEMPLATE_THEME_CONTENT_KEY] as Record<string, unknown> | undefined),
              ...themeSettings,
            },
          },
        }
...
export const supportsFrontendTemplateEditor = (templateId?: string | null): boolean =>
  !!templateId && LOCAL_TEMPLATE_EDITOR_IDS.has(templateId);
...
export const buildFrontendTemplateEditorPages = (
  ...
  const data = buildFrontendTemplateBusinessData(templateId, {
    ...
    themeSettings: website.themeSettings || website.templateSnapshot?.themeSettings || undefined,
...
const mapPosts = (
  ...
  items.map((item, index) => ({
    ...
    category: readString(item, ['category', 'tag', 'icon'], fallbackPosts[index]?.category || 'Article'),
    author: readString(item, ['author', 'authorName'], fallbackPosts[index]?.author || ''),
    publishedAt: readString(item, ['publishedAt', 'date'], fallbackPosts[index]?.publishedAt || ''),
    slug: readString(item, ['slug'], fallbackPosts[index]?.slug || `editor-article-${index + 1}`),
    content: readString(item, ['content', 'body'], fallbackPosts[index]?.content || ''),
  }));
...
const findSectionContent = (
  ...
  const match = getAllBlocks(pages).find((block) => getCompatibleSectionKey(templateId, block) === sectionKey);
...
const findSectionBlock = (
  ...
  getAllBlocks(pages).find((block) => getCompatibleSectionKey(templateId, block) === sectionKey);
...
export const buildTemplatePreviewBusinessData = (
  ...
  const orderedSectionContentMap = getOrderedSectionContentMap(templateId, pages);
  const getSectionContent = (sectionKey: string): Record<string, unknown> =>
    orderedSectionContentMap.get(sectionKey) || findSectionContent(templateId, pages, sectionKey);
  ...
  if (templateId === 'company-executive') {
    ...
    const whyUsItems = readArray<Record<string, unknown>>(whyUs, ['features', 'items']);
    const processItems = readArray<Record<string, unknown>>(process, ['features', 'items']);
    const contactHeading = readString(contact, ['heading', 'title'], 'Drop us a line.');
    const contactDescription = readString(contact, ['description', 'subheading', 'body'], String(themedBase.description));
    const contactButton = readString(contact, ['buttonLabel', 'primaryCtaText', 'ctaText'], 'Contact Us');

    return {
      ...themedBase,
      tagline: readString(overview, ['heading', 'heroHeading', 'title'], String(themedBase.tagline || themedBase.name)),
      description: readString(about, ['body', 'description', 'subheading'], String(themedBase.description)),
      features: whyUsItems.length ? mapFeatureItems(whyUsItems) : themedBase.features,
      templateContent: {
        home: {
          blockId: overviewBlock?.id,
          heading: readString(overview, ['heading', 'heroHeading', 'title']),
          subheading: readString(overview, ['subheading', 'heroDescription', 'description', 'body']),
          ctaText: readString(overview, ['ctaText', 'heroCtaText', 'buttonText', 'buttonLabel'], contactButton),
          primaryCtaText: readString(overview, ['ctaText', 'heroCtaText', 'buttonText', 'buttonLabel'], contactButton),
          heroHeading: readString(overview, ['heading', 'heroHeading', 'title']),
          heroDescription: readString(overview, ['subheading', 'heroDescription', 'description', 'body']),
          heroCtaText: readString(overview, ['ctaText', 'heroCtaText', 'buttonText', 'buttonLabel'], contactButton),
          ...
          outerSectionStyle: getSectionStyleValue(overview, 'outerSectionStyle'),
        },
        about: {
        ...
        process: {
          ...
          subheading: readString(process, ['subheading', 'description', 'body']),
          ctaText: readString(process, ['ctaText', 'buttonLabel', 'primaryCtaText'], contactButton),
          items: processItems,
        ...
        contact: {
          ...
          descriptionStyle: contact.descriptionStyle || contact.bodyStyle || contact.subheadingStyle,
  ...
  if (templateId === 'company-premium') {
    ...
    const contactHeading = readString(contact, ['heading', 'title'], "Let's shape something memorable.");
    const contactDescription = readString(contact, ['description', 'subheading', 'body'], 'Contact');
    const contactButton = readString(contact, ['buttonLabel', 'primaryCtaText', 'ctaText'], 'Book a consultation');

    return {
      ...themedBase,
      tagline: readString(about, ['heading', 'heroHeading', 'title'], String(themedBase.tagline || themedBase.name)),
      description: readString(work, ['body', 'description', 'subheading'], String(themedBase.description)),
      features: serviceItems.length ? mapFeatureItems(serviceItems) : themedBase.features,
      templateContent: {
        home: {
          blockId: aboutBlock?.id,
          heading: readString(about, ['heading', 'heroHeading', 'title']),
          subheading: readString(about, ['subheading', 'heroDescription', 'description', 'body']),
          ctaText: readString(about, ['ctaText', 'heroCtaText', 'buttonText', 'buttonLabel'], contactButton),
          primaryCtaText: readString(about, ['ctaText', 'heroCtaText', 'buttonText', 'buttonLabel'], contactButton),
          heroHeading: readString(about, ['heading', 'heroHeading', 'title']),
          heroDescription: readString(about, ['subheading', 'heroDescription', 'description', 'body']),
          heroCtaText: readString(about, ['ctaText', 'heroCtaText', 'buttonText', 'buttonLabel'], contactButton),
          headingStyle: about.headingStyle || about.titleStyle,
          subheadingStyle: about.subheadingStyle || about.descriptionStyle || about.bodyStyle,
        ...
        features: {
          ...
          description: readString(services, ['description', 'subheading', 'body']),
          headingStyle: services.headingStyle || services.titleStyle,
          descriptionStyle: services.descriptionStyle || services.subheadingStyle || services.bodyStyle,
          sectionStyle: getSectionStyleValue(services),
        },
        gallery: {
          blockId: galleryBlock?.id,
          heading: readString(gallery, ['title', 'heading']),
          description: readString(gallery, ['description', 'subheading', 'body']),
          headingStyle: gallery.headingStyle || gallery.titleStyle,
          descriptionStyle: gallery.descriptionStyle || gallery.subheadingStyle || gallery.bodyStyle,
        ...
        about: {
          ...
          descriptionStyle: work.descriptionStyle || work.bodyStyle || work.subheadingStyle,
  ...
  if (templateId === 'company') {
    ...
    const contact = getSectionContent('contact');
    const studioItems = readArray<Record<string, unknown>>(studio, ['features', 'items']);
    const contactHeading = readString(contact, ['heading', 'title'], 'Newsletter Sign Up');
    const contactDescription = readString(contact, ['description', 'subheading', 'body'], '');
    const contactButton = readString(contact, ['buttonLabel', 'primaryCtaText', 'ctaText'], 'Sign Up');

    return {
      ...themedBase,
      tagline: readString(projects, ['heading', 'heroHeading', 'title'], String(themedBase.tagline || themedBase.name)),
      description: readString(studio, ['body', 'description', 'subheading'], String(themedBase.description)),
      features: studioItems.length ? mapFeatureItems(studioItems) : themedBase.features,
      templateContent: {
        home: {
          blockId: projectsBlock?.id,
          heading: readString(projects, ['heading', 'heroHeading', 'title']),
          subheading: readString(projects, ['subheading', 'heroDescription', 'description', 'body']),
          ctaText: readString(projects, ['ctaText', 'heroCtaText', 'buttonText', 'buttonLabel'], contactButton),
          primaryCtaText: readString(projects, ['ctaText', 'heroCtaText', 'buttonText', 'buttonLabel'], contactButton),
          heroHeading: readString(projects, ['heading', 'heroHeading', 'title']),
          heroDescription: readString(projects, ['subheading', 'heroDescription', 'description', 'body']),
          heroCtaText: readString(projects, ['ctaText', 'heroCtaText', 'buttonText', 'buttonLabel'], contactButton),
          headingStyle: projects.headingStyle || projects.titleStyle,
          subheadingStyle: projects.subheadingStyle || projects.descriptionStyle || projects.bodyStyle,
        ...
        features: {
          ...
          description: readString(studio, ['body', 'description', 'subheading']),
          headingStyle: studio.headingStyle || studio.titleStyle,
          descriptionStyle: studio.descriptionStyle || studio.bodyStyle || studio.subheadingStyle,
  ...
  if (templateId === 'blog-premium') {
    ...
    const contact = getSectionContent('contact');
    const featureItems = readArray<Record<string, unknown>>(articles, ['features', 'items']);
    const staticPosts = readArray<Record<string, unknown>>(articles, ['staticPosts', 'posts', 'insights']);
    ...
    const contactHeading = readString(contact, ['heading', 'title'], 'Contact Us');
    const contactDescription = readString(contact, ['description', 'subheading', 'body'], '');
    const contactButton = readString(contact, ['buttonLabel', 'primaryCtaText', 'ctaText'], 'Contact');

    return {
      ...themedBase,
      description: readString(about, ['body', 'description', 'subheading'], readString(home, ['subheading', 'heroDescription', 'description', 'body'], String(themedBase.description))),
      blogPosts: derivedPosts,
      templateContent: {
        home: {
          blockId: homeBlock?.id,
          heroHeading: readString(home, ['heading', 'heroHeading', 'title']),
          heroDescription: readString(home, ['subheading', 'heroDescription', 'description', 'body']),
          heroCtaText: readString(home, ['ctaText', 'heroCtaText', 'buttonText', 'buttonLabel']),
          headingStyle: home.headingStyle || home.titleStyle,
          descriptionStyle: home.descriptionStyle || home.subheadingStyle || home.bodyStyle,
        ...
        about: {
          ...
          body: readString(about, ['body', 'description', 'subheading']),
          headingStyle: about.headingStyle || about.titleStyle,
          descriptionStyle: about.descriptionStyle || about.bodyStyle || about.subheadingStyle,
          sectionStyle: getSectionStyleValue(about),
        },
        contact: {
          blockId: contactBlock?.id,
  ...
  const derivedPosts = staticPosts.length
    ...
    : featureItems.length
      ? mapPosts(featureItems, themedBase.blogPosts || [])
      : themedBase.blogPosts || [];
  const contactHeading = readString(contact, ['heading', 'title'], 'Subscribe for updates');
  const contactDescription = readString(contact, ['description', 'subheading', 'body'], '');
  const contactButton = readString(contact, ['buttonLabel', 'primaryCtaText', 'ctaText'], 'Subscribe');

  return {
    ...themedBase,
    description: readString(home, ['subheading', 'heroDescription', 'description', 'body'], String(themedBase.description)),
    blogPosts: derivedPosts,
    templateContent: {
      home: {
        blockId: homeBlock?.id,
