...
describe('NavbarBlock', () => {
  ...
  it('caps nav items at 8', () => {
    const manyItems = Array.from({ length: 12 }, (_, i) => ({ label: `Item${i}`, link: `/i${i}` }));
    const { container } = render(
      <NavbarBlock block={makeNavbarBlock({ brandName: 'Site', navigationItems: manyItems })} />
    );
  ...
  it('falls back to "My Brand" when no brandName or logo', () => {
    render(
      <NavbarBlock block={makeNavbarBlock({ navigationItems: [{ label: 'Home', link: '/' }] })} />
    );
    expect(screen.getByText('My Brand')).toBeTruthy();
  });

  it('does not crash with empty content', () => {
    expect(() => render(<NavbarBlock block={makeNavbarBlock({})} />)).not.toThrow();
  });
  ...
  it('applies no blur when backdropBlur is "none"', () => {
    const { container } = render(
      <NavbarBlock
        block={makeNavbarBlock({ brandName: 'Site', navigationItems: [], backdropBlur: 'none' })}
      />
    );
  ...
  it('applies backdrop-filter blur(4px) for backdropBlur="sm"', () => {
    const { container } = render(
      <NavbarBlock
        block={makeNavbarBlock({ brandName: 'Site', navigationItems: [], backdropBlur: 'sm' })}
      />
    );
    const nav = container.querySelector('nav') as HTMLElement;
    expect(nav?.getAttribute('data-backdrop-blur')).toBe('sm');
  ...
  it('applies backdrop-filter blur(8px) for backdropBlur="md"', () => {
    const { container } = render(
      <NavbarBlock
        block={makeNavbarBlock({ brandName: 'Site', navigationItems: [], backdropBlur: 'md' })}
      />
    );
    const nav = container.querySelector('nav') as HTMLElement;
    expect(nav?.getAttribute('data-backdrop-blur')).toBe('md');
  });

  it('applies backdrop-filter blur(16px) for backdropBlur="lg"', () => {
    const { container } = render(
      <NavbarBlock
        block={makeNavbarBlock({ brandName: 'Site', navigationItems: [], backdropBlur: 'lg' })}
      />
    );
    const nav = container.querySelector('nav') as HTMLElement;
    expect(nav?.getAttribute('data-backdrop-blur')).toBe('lg');
  });
});
...
describe('FooterBlock', () => {
  it('renders copyright text', () => {
    render(<FooterBlock block={makeFooterBlock({ copyright: '© 2026 Acme Corp.' })} />);
  ...
  it('caps columns at 4', () => {
    const manyCols = Array.from({ length: 6 }, (_, i) => ({ title: `Col${i}`, links: [] }));
    render(<FooterBlock block={makeFooterBlock({ copyright: '© 2026', columns: manyCols })} />);
  ...
  it('caps social links at 5', () => {
    ...
    const { container } = render(
      <FooterBlock block={makeFooterBlock({ copyright: '© 2026', socialLinks: manySocial })} />
    );
  ...
  it('renders logo img when logo is provided', () => {
    ...
    const img = container.querySelector('img');
    expect(img).toBeTruthy();
    expect(img?.getAttribute('src')).toBe('https://example.com/footer-logo.png');
  });
  ...
  it('does not crash with empty content', () => {
    expect(() => render(<FooterBlock block={makeFooterBlock({})} />)).not.toThrow();
  });

  it('has data-block-type="FOOTER" attribute', () => {
    const { container } = render(<FooterBlock block={makeFooterBlock({ copyright: '© 2026' })} />);
