...
const makeBlock = (blockType: string, content: Record<string, unknown> = {}): PreviewBlock => ({
  id: 1,
  blockType,
  content,
  sortOrder: 0,
...
describe('PreviewRenderer', () => {
  const blocks: PreviewBlock[] = [
    { id: 1, blockType: 'HERO', content: { heading: 'Hello' }, sortOrder: 0, isVisible: true },
    { id: 2, blockType: 'TEXT', content: { title: 'About' }, sortOrder: 1, isVisible: true },
    { id: 3, blockType: 'CTA', content: { heading: 'Go' }, sortOrder: 2, isVisible: true },
  ];
  ...
  it('wraps in preview-wrapper with viewport data attribute', () => {
    const { container } = render(<PreviewRenderer blocks={blocks} viewport="mobile" />);
  ...
  it('filters out invisible blocks', () => {
    const blocksWithHidden: PreviewBlock[] = [
      { id: 1, blockType: 'HERO', content: { heading: 'Hello' }, sortOrder: 0, isVisible: true },
      { id: 2, blockType: 'TEXT', content: { title: 'Hidden' }, sortOrder: 1, isVisible: false },
    ];
  ...
  it('applies tablet viewport width', () => {
    const { container } = render(<PreviewRenderer blocks={blocks} viewport="tablet" />);
    const wrapper = container.querySelector('.preview-wrapper');
    expect(wrapper?.getAttribute('style')).toContain('768px');
  });

  it('sets siteName as data attribute', () => {
    const { container } = render(<PreviewRenderer blocks={blocks} siteName="TestSite" />);
    const wrapper = container.querySelector('.preview-wrapper');
