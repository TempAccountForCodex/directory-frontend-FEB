...
const makeBlock = (blockType: string, content: Record<string, unknown> = {}) => ({
  id: 1,
  blockType,
  content,
  sortOrder: 0,
...
describe('HeroBlock', () => {
  it('renders heading and subheading with correct classes', () => {
    const { container } = render(
      <HeroBlock block={makeBlock('HERO', { heading: 'Test Title', subheading: 'Sub text' })} />
    );
    expect(container.querySelector('.block--hero')).toBeTruthy();
    expect(container.querySelector('.hero__inner')).toBeTruthy();
    expect(container.querySelector('.hero__heading')?.textContent).toBe('Test Title');
    expect(container.querySelector('.hero__subheading')?.textContent).toBe('Sub text');
  });

  it('renders CTA as link when not in preview', () => {
    const { container } = render(
      <HeroBlock
        block={makeBlock('HERO', { heading: 'H', ctaText: 'Click', ctaLink: '/go' })}
        isPreview={false}
      />
    );
  ...
  it('defaults heading to Welcome', () => {
    ...
    expect(container.querySelector('.hero__heading')?.textContent).toBe('Welcome');
  });

  it('escapes HTML in heading', () => {
    const { container } = render(
      <HeroBlock block={makeBlock('HERO', { heading: '<script>alert(1)</script>' })} />
    );
    expect(container.querySelector('.hero__heading')?.innerHTML).not.toContain('<script>');
  });

  it('applies subtle text-shadow to heading', () => {
    const { container } = render(
      <HeroBlock block={makeBlock('HERO', { heading: 'H', headingTextShadow: 'subtle' })} />
    );
  ...
  it('applies dramatic text-shadow to heading', () => {
    const { container } = render(
      <HeroBlock block={makeBlock('HERO', { heading: 'H', headingTextShadow: 'dramatic' })} />
    );
    const h1 = container.querySelector('.hero__heading') as HTMLElement;
  ...
  it('applies opacity to heading when headingOpacity < 100', () => {
    const { container } = render(
      <HeroBlock block={makeBlock('HERO', { heading: 'H', headingOpacity: 70 })} />
    );
    const h1 = container.querySelector('.hero__heading') as HTMLElement;
  ...
  it('does not apply opacity style when headingOpacity is 100', () => {
    const { container } = render(
      <HeroBlock block={makeBlock('HERO', { heading: 'H', headingOpacity: 100 })} />
    );
    const h1 = container.querySelector('.hero__heading') as HTMLElement;
  ...
  it('applies tight line-height to heading', () => {
    const { container } = render(
      <HeroBlock block={makeBlock('HERO', { heading: 'H', headingLineHeight: 'tight' })} />
    );
    const h1 = container.querySelector('.hero__heading') as HTMLElement;
  ...
  it('applies relaxed line-height to heading', () => {
    const { container } = render(
      <HeroBlock block={makeBlock('HERO', { heading: 'H', headingLineHeight: 'relaxed' })} />
    );
    const h1 = container.querySelector('.hero__heading') as HTMLElement;
...
describe('TextBlock', () => {
  it('renders title and body with correct classes', () => {
    const { container } = render(
      <TextBlock block={makeBlock('TEXT', { title: 'Title', body: '<b>bold</b>' })} />
    );
    ...
    expect(container.querySelector('.text__body')?.innerHTML).toContain('<b>bold</b>');
  });
  ...
  it('applies text alignment', () => {
    const { container } = render(<TextBlock block={makeBlock('TEXT', { alignment: 'right' })} />);
...
describe('ImageBlock', () => {
  it('renders image with figure and correct width class', () => {
    const { container } = render(
      <ImageBlock block={makeBlock('IMAGE', { src: '/img.jpg', alt: 'Test', width: 'medium' })} />
    );
    expect(container.querySelector('.block--image')).toBeTruthy();
    expect(container.querySelector('.image__figure--medium')).toBeTruthy();
    expect(container.querySelector('.image__img')?.getAttribute('alt')).toBe('Test');
    expect(container.querySelector('.image__img')?.getAttribute('loading')).toBe('lazy');
  });

  it('renders caption when provided', () => {
    const { container } = render(
      <ImageBlock block={makeBlock('IMAGE', { src: '/img.jpg', caption: 'A caption' })} />
    );
    expect(container.querySelector('.image__caption')?.textContent).toBe('A caption');
  });
  ...
  it('blocks javascript: URLs', () => {
    ...
    expect(container.querySelector('.image__img')?.getAttribute('src')).toBeNull();
  });
});

describe('FormBlock', () => {
  it('renders form with disabled inputs', () => {
    const { container } = render(<FormBlock block={makeBlock('FORM', { title: 'Contact' })} />);
    ...
    expect(container.querySelector('.form__submit')?.getAttribute('disabled')).toBe('');
  });

  it('renders contact info when email and phone provided', () => {
    const { container } = render(
      <FormBlock block={makeBlock('FORM', { title: 'Contact', email: 'a@b.com', phone: '123' })} />
    );
    expect(container.querySelector('.form__contact-info')).toBeTruthy();
    expect(container.querySelector('.form__contact-link')?.getAttribute('href')).toBe(
      'mailto:a@b.com'
    );
  });
  ...
  it('defaults title to Contact Us', () => {
    ...
    expect(container.querySelector('.form__title')?.textContent).toBe('Contact Us');
  });
});
...
describe('GalleryBlock', () => {
  ...
  it('shows empty message when no images', () => {
    const { container } = render(<GalleryBlock block={makeBlock('GALLERY', {})} />);
    expect(container.querySelector('.gallery__empty')?.textContent).toBe('No images');
  });

  it('limits to 20 images', () => {
    const images = Array.from({ length: 25 }, (_, i) => ({ src: `/${i}.jpg`, alt: `${i}` }));
    const { container } = render(<GalleryBlock block={makeBlock('GALLERY', { images })} />);
...
describe('CtaBlock', () => {
  it('renders heading, subheading, and buttons', () => {
    ...
    expect(container.querySelector('.cta__heading')?.textContent).toBe('Act Now');
    expect(container.querySelector('.cta__btn--primary')?.textContent).toBe('Buy');
    expect(container.querySelector('.cta__btn--secondary')?.textContent).toBe('Learn');
  });

  it('renders buttons as spans in preview mode', () => {
    const { container } = render(
      <CtaBlock
        block={makeBlock('CTA', { primaryCtaText: 'Go', primaryCtaLink: '/go' })}
        isPreview={true}
      />
    );
  ...
  it('applies medium text-shadow to CTA heading', () => {
    const { container } = render(
      <CtaBlock block={makeBlock('CTA', { heading: 'H', headingTextShadow: 'medium' })} />
    );
  ...
  it('applies opacity to CTA heading when headingOpacity < 100', () => {
    const { container } = render(
      <CtaBlock block={makeBlock('CTA', { heading: 'H', headingOpacity: 50 })} />
    );
    const h2 = container.querySelector('.cta__heading') as HTMLElement;
  ...
  it('applies normal line-height 1.2 to CTA heading', () => {
    const { container } = render(
      <CtaBlock block={makeBlock('CTA', { heading: 'H', headingLineHeight: 'normal' })} />
    );
    const h2 = container.querySelector('.cta__heading') as HTMLElement;
  ...
  it('does not apply heading style when all defaults', () => {
    const { container } = render(
      <CtaBlock
        block={makeBlock('CTA', { heading: 'H', headingTextShadow: 'none', headingOpacity: 100 })}
      />
    );
    const h2 = container.querySelector('.cta__heading') as HTMLElement;
...
describe('TestimonialsBlock', () => {
  it('renders testimonial cards with correct structure (items key)', () => {
    ...
    expect(container.querySelector('.testimonials__title')?.textContent).toBe('Reviews');
    expect(container.querySelectorAll('.testimonial__card')).toHaveLength(2);
    expect(container.querySelector('.testimonial__quote')?.textContent).toContain('Great!');
    expect(container.querySelector('.testimonial__author')?.textContent).toBe('Jane');
    expect(container.querySelector('.testimonial__role')?.textContent).toBe('CEO');
  });
  ...
  it('falls back to testimonials key when items absent', () => {
    ...
    expect(container.querySelector('.testimonial__quote')?.textContent).toContain('Fallback');
  });
  ...
  it('shows placeholder avatar when no image', () => {
    ...
    const placeholder = container.querySelector('.testimonial__avatar--placeholder');
    expect(placeholder?.textContent).toBe('J');
  });

  it('shows empty message when no testimonials', () => {
    const { container } = render(<TestimonialsBlock block={makeBlock('TESTIMONIALS', {})} />);
  ...
  it('renders stacked variant when variant="stacked"', () => {
    ...
    expect(container.querySelector('.block--testimonials-stacked')).toBeTruthy();
  });
  ...
  it('stacked variant shows a single slide at a time', () => {
    ...
    expect(container.querySelectorAll('.testimonial__card--stacked')).toHaveLength(1);
    expect(container.querySelector('.testimonial__quote')?.textContent).toContain('First');
  });
  ...
  it('stacked variant advances on next arrow click', async () => {
    ...
    expect(container.querySelector('.testimonial__quote')?.textContent).toContain('Alpha');
    await user.click(container.querySelector('.testimonials__arrow--next')!);
    expect(container.querySelector('.testimonial__quote')?.textContent).toContain('Beta');
  });
  ...
  it('stacked variant goes back on prev arrow click (wraps)', async () => {
    ...
    expect(container.querySelector('.testimonial__quote')?.textContent).toContain('Last');
  });
  ...
  it('stacked variant navigates via dot click', async () => {
    ...
    expect(container.querySelector('.testimonial__quote')?.textContent).toContain('Three');
  });
  ...
  it('stacked variant auto-rotates after 6s', () => {
    ...
    expect(container.querySelector('.testimonial__quote')?.textContent).toContain('Second');
    vi.useRealTimers();
  });
});
...
describe('FeaturesBlock', () => {
  it('renders features grid with correct column class (items key)', () => {
    ...
    expect(container.querySelector('.features__title')?.textContent).toBe('Features');
    expect(container.querySelector('.features__subtitle')?.textContent).toBe('Why choose us');
  });
  ...
  it('falls back to features key when items absent', () => {
    ...
    expect(container.querySelector('.feature__title')?.textContent).toBe('Fallback Feature');
  });

  it('shows empty message when no features', () => {
    const { container } = render(<FeaturesBlock block={makeBlock('FEATURES', {})} />);
    expect(container.querySelector('.features__empty')?.textContent).toBe('No features listed.');
  });

  it('limits to 12 features', () => {
    const items = Array.from({ length: 15 }, (_, i) => ({ title: `Feature ${i}` }));
    const { container } = render(<FeaturesBlock block={makeBlock('FEATURES', { items })} />);
...
describe('DefaultBlock', () => {
  it('renders with correct class and data attribute', () => {
    const { container } = render(<DefaultBlock block={makeBlock('UNKNOWN_TYPE', {})} />);
    expect(container.querySelector('.block--default')).toBeTruthy();
    expect(container.querySelector('[data-block-type="UNKNOWN_TYPE"]')).toBeTruthy();
  });
});
