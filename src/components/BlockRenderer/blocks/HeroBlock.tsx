...
const LINE_HEIGHT_MAP: Record<string, number> = { tight: 0.9, normal: 1.2, relaxed: 1.5 };
...
const HeroBlock: React.FC<BlockRendererProps> = ({ block, isPreview }) => {
  ...
  const styles: string[] = [];
  if (c.backgroundColor) styles.push(`background-color:${escapeAttr(c.backgroundColor)}`);
  ...
  if (alignment !== 'center') styles.push(`text-align:${escapeAttr(alignment)}`);

  const styleAttr = styles.length > 0 ? styles.join(';') : undefined;

  const headingStyle: React.CSSProperties = {};
  if (c.headingTextShadow && c.headingTextShadow !== 'none' && SHADOW_MAP[c.headingTextShadow]) {
  ...
  if (c.headingLineHeight && LINE_HEIGHT_MAP[c.headingLineHeight] !== undefined) {
  ...
  return (
    <section
      ...
      style={styleAttr ? ({ cssText: styleAttr } as unknown as React.CSSProperties) : undefined}
