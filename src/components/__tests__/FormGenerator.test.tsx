 ...
 */
const MockField: React.FC<FieldRendererProps> = ({ field, value, onChange, disabled, errors }) =>
    ...
    (
      ...
      React.createElement(
        'div',
        {
          ...
          'data-animate':
            typeof animate === 'object' ? JSON.stringify(animate) : String(animate ?? ''),
          'data-exit': typeof exit === 'object' ? JSON.stringify(exit) : String(exit ?? ''),
          style,
          className,
          ...rest,
        },
 ...
 */
const MockToggle: React.FC<FieldRendererProps> = ({ field, value, onChange, disabled, errors }) =>
  React.createElement('input', {
    type: 'checkbox',
    'data-testid': `field-${field.name}`,
    'data-disabled': disabled ? 'true' : 'false',
...
describe('FormGenerator', () => {
  ...
  it('hides fields whose shouldShowField condition evaluates false', () => {
    ...
    render(<FormGenerator blockType="hero" initialValues={{ showExtra: false }} />);
  ...
  it('shows fields when shouldShowField condition evaluates true', () => {
    makeReadyState(conditionalMetadata as typeof basicMetadata);
    render(<FormGenerator blockType="hero" initialValues={{ showExtra: true }} />);
    expect(screen.getByTestId('field-showExtra')).toBeInTheDocument();
  ...
  it('populates fields from initialValues on mount', () => {
    ...
    expect(screen.getByTestId('field-subtitle')).toHaveValue('Initial Subtitle');
  });
  ...
  it('fires onChange immediately — not after a debounce delay', async () => {
    ...
    const handleChange = vi.fn();
    render(
      <FormGenerator blockType="hero" initialValues={{ title: '' }} onChange={handleChange} />
    );
  ...
  it('does not call onValidate multiple times during rapid field updates — only once after debounce', async () => {
    ...
    expect(onValidate.mock.calls.length).toBeLessThanOrEqual(callCountAfterMount + 1);
  });
  ...
  it('passes errors to FieldRenderer via errors prop after blur triggers touched state', async () => {
    makeReadyState(requiredFieldMetadata as typeof basicMetadata);
    render(<FormGenerator blockType="hero" initialValues={{ requiredField: '' }} />);
  ...
  it('blocks onSubmit when validation fails (form has errors)', async () => {
    ...
    render(
      <FormGenerator blockType="hero" initialValues={{ requiredField: '' }} onSubmit={onSubmit} />
    );
  ...
  it('[PROD QA] does not crash and ignores patterns exceeding 500 chars', async () => {
    ...
    expect(() =>
      render(<FormGenerator blockType="hero" initialValues={{ field1: 'any value' }} />)
    ).not.toThrow();
  ...
  it('[PROD QA] does not crash when blockType contains special characters', () => {
    makeLoadingState();
    expect(() => render(<FormGenerator blockType="hero/admin" />)).not.toThrow();
  });
  ...
  it('[PROD QA] getFieldHandler returns the same function reference across re-renders', () => {
    makeReadyState(basicMetadata);
    const handleChange = vi.fn();
    const { rerender } = render(<FormGenerator blockType="hero" onChange={handleChange} />);
  ...
  describe('Step 2.8.1 — Conditional Field Animations', () => {
    ...
    it('renders AnimatePresence around field groups', () => {
      makeReadyState(basicMetadata);
      render(<FormGenerator blockType="hero" />);
      const presence = document.querySelectorAll('[data-animate-presence="true"]');
  ...
  describe('Step 2.8.2 — clearOnHide Value Management', () => {
    ...
    it('clears value when field with clearOnHide=true is hidden', async () => {
      ...
      render(
        <FormGenerator
          blockType="hero"
          initialValues={{ toggle: true, clearable: 'keep me', preserved: 'also keep' }}
          onChange={handleChange}
        />
      );
    ...
    it('value is restored to default when field with clearOnHide becomes visible again', async () => {
      ...
      render(
        <FormGenerator
          blockType="hero"
          initialValues={{ toggle: true, clearable: 'original', preserved: 'val2' }}
        />
      );
