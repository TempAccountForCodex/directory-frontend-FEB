...
function PermissionGateBooleans() {
  ...
  return (
    <div>
      ...
      <span data-testid="canManageCollaborators">{String(perms.canManageCollaborators)}</span>
      <span data-testid="canAccessDashboard">{String(perms.canAccessDashboard)}</span>
      <span data-testid="canViewAnalytics">{String(perms.canViewAnalytics)}</span>
      <span data-testid="canManageForms">{String(perms.canManageForms)}</span>
      <span data-testid="canManageIntegrations">{String(perms.canManageIntegrations)}</span>
...
describe('PermissionGate (Step 7.2.4)', () => {
  ...
  it('usePermissionGate returns all 9 booleans correctly for OWNER', () => {
    ...
    expect(screen.getByTestId('canManageCollaborators')).toHaveTextContent('true');
    ...
    expect(screen.getByTestId('canManageIntegrations')).toHaveTextContent('true');
  ...
  it('usePermissionGate returns correct booleans for EDITOR', () => {
    ...
    expect(screen.getByTestId('canManageCollaborators')).toHaveTextContent('false');
    expect(screen.getByTestId('canAccessDashboard')).toHaveTextContent('true');
    expect(screen.getByTestId('canViewAnalytics')).toHaveTextContent('true');
    expect(screen.getByTestId('canManageForms')).toHaveTextContent('true');
    expect(screen.getByTestId('canManageIntegrations')).toHaveTextContent('false');
