...
vi.mock('../../Templates/PreviewSkeleton', () => ({
  PreviewImageError: ({ onRetry, message }: { onRetry?: () => void; message?: string }) => (
  ...
  PreviewNetworkError: ({ onRetry }: { onRetry?: () => void }) => (
    <div data-testid="network-error">{onRetry && <button onClick={onRetry}>Try Again</button>}</div>
  ),
...
const mockGenerateLivePreview = vi.fn(() => '<html><body>Live preview</body></html>');
...
describe('PreviewPanel', () => {
  beforeEach(() => {
    ...
    mockGenerateLivePreview.mockReturnValue('<html><body>Live preview</body></html>');
  });
  ...
  it('shows error state when iframe fails in static mode', () => {
    mockIframeReturn = { ...mockIframeReturn, iframeError: true, iframeLoading: false };
    render(<PreviewPanel websiteId="1" pageId="1" />);
  ...
  it('renders mode toggle with Live and Static options', () => {
    render(<PreviewPanel websiteId="1" pageId="1" />);
    expect(screen.getByRole('button', { name: /live mode/i })).toBeInTheDocument();
    expect(screen.getByRole('button', { name: /static mode/i })).toBeInTheDocument();
  });
  ...
  it('shows token expired banner when tokenExpired is true in static mode', () => {
    mockIframeReturn = { ...mockIframeReturn, tokenExpired: true, iframeLoading: false };
    render(<PreviewPanel websiteId="1" pageId="1" />);
