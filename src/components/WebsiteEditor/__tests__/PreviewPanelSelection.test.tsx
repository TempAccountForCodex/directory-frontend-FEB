...
const mockPreviewContent = {
  websiteId: '1',
  pageId: 'page-1',
  blocks: [{ id: 'hero-1', blockType: 'HERO', content: { title: 'Test' }, order: 0 }],
...
function dispatchPostMessage(data: Record<string, unknown>, origin = window.location.origin) {
...
describe('PreviewPanel selection extensions (Step 9.14.3)', () => {
  ...
  it('calls onBlockSelected when BLOCK_SELECTED postMessage received', () => {
    ...
    render(<PreviewPanel websiteId="1" pageId="1" onBlockSelected={onBlockSelected} />);
  ...
  it('calls onBlockHover when BLOCK_HOVER postMessage received', () => {
    ...
    render(<PreviewPanel websiteId="1" pageId="1" onBlockHover={onBlockHover} />);
  ...
  it('sends SELECT_BLOCK to iframe when selectedBlockId changes to non-null', () => {
    ...
    const { rerender } = render(<PreviewPanel websiteId="1" pageId="1" selectedBlockId={null} />);
    ...
    rerender(<PreviewPanel websiteId="1" pageId="1" selectedBlockId="hero-1" />);

    // Check that postMessage was called with SELECT_BLOCK
    const selectCall = mockPostMessage.mock.calls.find(
      (call: unknown[]) => (call[0] as Record<string, unknown>).type === 'SELECT_BLOCK'
    );
  ...
  it('sends DESELECT_ALL to iframe when selectedBlockId changes to null', () => {
    ...
    const { rerender } = render(<PreviewPanel websiteId="1" pageId="1" selectedBlockId="hero-1" />);
    ...
    const deselectCall = mockPostMessage.mock.calls.find(
      (call: unknown[]) => (call[0] as Record<string, unknown>).type === 'DESELECT_ALL'
    );
