...
import { Menu as MenuIcon, BookOpen as BookOpenIcon, ChevronRight } from 'lucide-react';
...
const SidebarContent = memo<SidebarContentProps>(({ sections, currentPath, onClose }) => {
  return (
    <Box
        ...
        <nav data-testid="docs-nav" aria-label="Documentation navigation">
          ...
          <List dense disablePadding>
            {sections.map((section) => {
              const isActive = currentPath.includes(`/docs/category/${section.slug}`);
              return (
                <ListItemButton
                  key={section.slug}
                  component={Link}
...
const DocsLayout = memo<DocsLayoutProps>(({ children, sections = DEFAULT_SECTIONS }) => {
    ...
    <Box sx={{ display: 'flex', minHeight: '100vh', bgcolor: 'background.default' }}>
      ...
      {!isMobile && (
        <Box sx={{ flexShrink: 0, width: SIDEBAR_WIDTH }}>
          <Box sx={{ position: 'fixed', top: 64, height: 'calc(100vh - 64px)', overflowY: 'auto' }}>
            <SidebarContent sections={sections} currentPath={location.pathname} />
          </Box>
        </Box>
      )}
