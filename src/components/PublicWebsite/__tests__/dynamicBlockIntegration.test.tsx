...
import { DynamicBlockProvider, DynamicBlockContext } from '../../../context/DynamicBlockContext';
...
const WithProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => (
...
describe('DynamicBlockRenderer — Full Integration Tests', () => {
  ...
  it('renders static HERO block via BlockRenderer (no skeleton, no hook)', () => {
    ...
    expect(screen.getByTestId('block-renderer')).toHaveAttribute('data-block-type', 'HERO');
  ...
  it('renders BLOG_FEED dynamic block with fetched data merged into content', () => {
    const blogData = { insights: [{ id: 1, title: 'Test Post' }], pagination: { totalPages: 1 } };
  ...
  it('renders DIRECTORY_LISTING dynamic block with listing data', () => {
    const listingData = { listings: [{ id: 1, name: 'TechCorp' }], totalCount: 1 };
  ...
  it('treats EVENTS_LIST as a dynamic block (in DYNAMIC_BLOCK_TYPES set)', () => {
    ...
    expect(screen.getByTestId('block-renderer')).toHaveAttribute('data-block-type', 'EVENTS_LIST');
    // useDynamicBlockData IS called (dynamic path), but with null dataSource
  ...
  it('DynamicBlockContext hydrates from window.__DYNAMIC_BLOCK_DATA__ on init', () => {
    (window as any).__DYNAMIC_BLOCK_DATA__ = {
      100: { insights: [{ id: 99, title: 'SSR Post' }], pagination: { totalPages: 1 } },
    };
    ...
    const ssrData = { insights: [{ id: 99, title: 'SSR Post' }], pagination: { totalPages: 1 } };
  ...
  it('triggers client-side fetch when no SSR data is available', () => {
    ...
    expect(useDynamicBlockData).toHaveBeenCalledWith(80, 'BLOG_FEED', '/api/blocks/80/data');
  });
  ...
  it('renders BlockRenderer with empty data (no items) without crashing', () => {
    ...
    expect(screen.getByTestId('block-renderer')).toBeInTheDocument();
    const content = JSON.parse(screen.getByTestId('block-renderer').textContent || '{}');
  ...
  it('renders a mixed page of static and dynamic blocks without conflicts', () => {
    ...
    const blockTypes = renderers.map((el) => el.getAttribute('data-block-type'));
  ...
  it('useDynamicBlockData is called with blockId, blockType, and dataSource', () => {
    ...
    expect(useDynamicBlockData).toHaveBeenCalledWith(42, 'BLOG_FEED', '/api/custom-feed');
  });
  ...
  it('DynamicBlockRenderer is defined and wrapped in React.memo', () => {
    ...
    expect((DynamicBlockRenderer as any).displayName).toBe('DynamicBlockRenderer');
  });
});
