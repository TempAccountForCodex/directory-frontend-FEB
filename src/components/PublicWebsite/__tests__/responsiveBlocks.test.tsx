...
const makeBlock = (blockType: string, id: number, extraContent: any = {}): any => ({
  id,
  blockType,
...
describe('Responsive Rendering — All Block Types', () => {
  ...
  describe('Dynamic blocks at 375px mobile — BlockSkeleton responsive', () => {
    ...
    it('BLOG_FEED skeleton renders without overflow at 375px', () => {
      const { container } = renderWithProviders(<BlockSkeleton blockType="BLOG_FEED" />);
      expect(container.firstChild).not.toBeNull();
      expect(screen.getByRole('status')).toBeInTheDocument();
    });

    it('BLOG_ARTICLE skeleton renders without overflow at 375px', () => {
      const { container } = renderWithProviders(<BlockSkeleton blockType="BLOG_ARTICLE" />);
      expect(container.firstChild).not.toBeNull();
    });

    it('PRODUCT_SHOWCASE skeleton renders without overflow at 375px', () => {
      const { container } = renderWithProviders(<BlockSkeleton blockType="PRODUCT_SHOWCASE" />);
      expect(container.firstChild).not.toBeNull();
    ...
    it('DIRECTORY_LISTING skeleton renders without overflow at 375px', () => {
      const { container } = renderWithProviders(<BlockSkeleton blockType="DIRECTORY_LISTING" />);
      expect(container.firstChild).not.toBeNull();
      const skeletons = container.querySelectorAll('.MuiSkeleton-root');
      expect(skeletons.length).toBeGreaterThan(0);
    });
    ...
    it('REVIEWS skeleton renders without overflow at 375px', () => {
      const { container } = renderWithProviders(<BlockSkeleton blockType="REVIEWS" />);
      expect(container.firstChild).not.toBeNull();
    });

    it('EVENTS_LIST skeleton renders without overflow at 375px', () => {
      const { container } = renderWithProviders(<BlockSkeleton blockType="EVENTS_LIST" />);
      expect(container.firstChild).not.toBeNull();
    });
  });
  ...
  describe('STEPS_PROCESS block responsive behavior', () => {
    ...
    it('STEPS_PROCESS renders at 768px tablet', () => {
      setViewport(768);
      renderWithProviders(<StepsProcessBlock block={stepsBlockFixture} {...colors} />);
  ...
  describe('Grid breakpoints in core static blocks', () => {
    it('FEATURES block Grid items use xs/md breakpoints (not fixed widths)', () => {
      ...
      expect(screen.getByTestId('block-renderer')).toHaveAttribute('data-block-type', 'FEATURES');
    });
    ...
    it('BlockSkeleton FEATURES renders 3-column grid skeleton', () => {
      const { container } = renderWithProviders(<BlockSkeleton blockType="FEATURES" />);
    ...
    it('BlockSkeleton TESTIMONIALS renders 2-column grid skeleton', () => {
      const { container } = renderWithProviders(<BlockSkeleton blockType="TESTIMONIALS" />);
