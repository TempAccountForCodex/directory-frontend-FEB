...
const BlockSkeleton: React.FC<BlockSkeletonProps> = ({ blockType, height }) => {
  const renderSkeleton = () => {
    switch (blockType) {
      case 'HERO':
        return (
          <Box sx={{ position: 'relative', height: height ?? 400, overflow: 'hidden' }}>
      ...
      case 'TEXT':
        return (
          <Container sx={{ py: 6 }}>
            <Skeleton variant="text" sx={{ fontSize: '2rem', mb: 2, width: '50%' }} />
        ...
        return (
          <Container sx={{ py: 6 }}>
            ...
            {Array.from({ length: 5 }).map((_, i) => (
              <Skeleton key={i} variant="text" sx={{ fontSize: '1.25rem', mb: 1 }} />
            ))}
          </Container>
        );
      ...
      case 'BLOG_FEED':
        return (
          <Container sx={{ py: 6 }}>
            <Skeleton variant="text" sx={{ fontSize: '2rem', mb: 1, width: '40%', mx: 'auto' }} />
            <Skeleton variant="text" sx={{ fontSize: '1rem', mb: 3, width: '60%', mx: 'auto' }} />
            {/* Search skeleton */}
            <Skeleton
              variant="rectangular"
              height={56}
  ...
  return (
    <Box role="status" aria-busy="true" aria-label={`Loading ${blockType} block`}>
      {renderSkeleton()}
    </Box>
  );
};
