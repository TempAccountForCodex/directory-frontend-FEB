...
describe("useResponsiveVisibility", () => {
  ...
  it("hideOnMobile hides xs and sm", () => {
    const { result } = renderHook(() =>
      useResponsiveVisibility({ hideOnMobile: true }),
    );
    expect(result.current.isHidden).toBe(false);
    const display = result.current.visibilitySx.display as Record<
      string,
      string
    >;
  ...
  it("hideOnTablet hides md", () => {
    const { result } = renderHook(() =>
      useResponsiveVisibility({ hideOnTablet: true }),
    );
    const display = result.current.visibilitySx.display as Record<
      string,
      string
  ...
  it("hideOnDesktop hides lg and xl", () => {
    const { result } = renderHook(() =>
      useResponsiveVisibility({ hideOnDesktop: true }),
    );
    const display = result.current.visibilitySx.display as Record<
      string,
      string
  ...
  it("result is memoized across re-renders with same fields", () => {
    ...
    const { result, rerender } = renderHook(() =>
      useResponsiveVisibility(fields),
    );
  ...
  it("no flags returns empty visibilitySx (not display object)", () => {
    const { result } = renderHook(() =>
      useResponsiveVisibility({ hideOnMobile: false }),
    );
    expect(result.current.visibilitySx).toEqual({});
  });
