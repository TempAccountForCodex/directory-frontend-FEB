...
describe('useBlockAnimation', () => {
  it('returns shouldAnimate=false for none', () => {
    const { result } = renderHook(() => useBlockAnimation({ animationEntranceType: 'none' }));
  ...
  it('fade-in maps to opacity 0→1', () => {
    const { result } = renderHook(() => useBlockAnimation({ animationEntranceType: 'fade-in' }));
  ...
  it('slide-up maps to y 40→0', () => {
    const { result } = renderHook(() => useBlockAnimation({ animationEntranceType: 'slide-up' }));
    expect((result.current.motionProps.initial as Record<string, unknown>).y).toBe(40);
    expect((result.current.motionProps.animate as Record<string, unknown>).y).toBe(0);
  });

  it('slide-down maps to y -40→0', () => {
    const { result } = renderHook(() => useBlockAnimation({ animationEntranceType: 'slide-down' }));
    expect((result.current.motionProps.initial as Record<string, unknown>).y).toBe(-40);
    expect((result.current.motionProps.animate as Record<string, unknown>).y).toBe(0);
  });

  it('scale-up maps to scale 0.8→1', () => {
    const { result } = renderHook(() => useBlockAnimation({ animationEntranceType: 'scale-up' }));
    expect((result.current.motionProps.initial as Record<string, unknown>).scale).toBe(0.8);
    expect((result.current.motionProps.animate as Record<string, unknown>).scale).toBe(1);
  });

  it('bounce uses spring transition', () => {
    const { result } = renderHook(() => useBlockAnimation({ animationEntranceType: 'bounce' }));
    const transition = result.current.motionProps.transition as Record<string, unknown>;
  ...
  it('converts animationDuration ms to seconds', () => {
    const { result } = renderHook(() =>
      useBlockAnimation({ animationEntranceType: 'fade-in', animationDuration: 600 })
    );
    const transition = result.current.motionProps.transition as Record<string, unknown>;
  ...
  it('converts animationDelay ms to seconds', () => {
    const { result } = renderHook(() =>
      useBlockAnimation({ animationEntranceType: 'fade-in', animationDelay: 200 })
    );
    const transition = result.current.motionProps.transition as Record<string, unknown>;
  ...
  it('scroll triggered uses whileInView', () => {
    ...
    expect(result.current.motionProps.viewport).toEqual({ once: true, margin: '-60px 0px' });
  ...
  it('slide-up direction=left uses x:40 offset', () => {
    const { result } = renderHook(() =>
      useBlockAnimation({ animationEntranceType: 'slide-up', animationDirection: 'left' })
    );
    const initial = result.current.motionProps.initial as Record<string, unknown>;
  ...
  it('slide-up direction=right uses x:-40 offset', () => {
    const { result } = renderHook(() =>
      useBlockAnimation({ animationEntranceType: 'slide-up', animationDirection: 'right' })
    );
    const initial = result.current.motionProps.initial as Record<string, unknown>;
    expect(initial.x).toBe(-40);
    expect(initial.y).toBeUndefined();
  ...
  it('slide-up direction=down uses y:-40 offset', () => {
    const { result } = renderHook(() =>
      useBlockAnimation({ animationEntranceType: 'slide-up', animationDirection: 'down' })
    );
    const initial = result.current.motionProps.initial as Record<string, unknown>;
  ...
  it('slide-down direction=up overrides to y:40', () => {
    const { result } = renderHook(() =>
      useBlockAnimation({ animationEntranceType: 'slide-down', animationDirection: 'up' })
    );
    const initial = result.current.motionProps.initial as Record<string, unknown>;
    expect(initial.y).toBe(40);
  });
  ...
  it('fade-in ignores animationDirection', () => {
    const { result } = renderHook(() =>
      useBlockAnimation({ animationEntranceType: 'fade-in', animationDirection: 'left' })
    );
    expect(result.current.motionProps.initial).toEqual({ opacity: 0 });
    expect(result.current.motionProps.animate).toEqual({ opacity: 1 });
  });
  ...
  it('scale-up ignores animationDirection', () => {
    const { result } = renderHook(() =>
      useBlockAnimation({ animationEntranceType: 'scale-up', animationDirection: 'right' })
    );
    const initial = result.current.motionProps.initial as Record<string, unknown>;
  ...
  it('bounce direction=left includes x:40 in initial', () => {
    const { result } = renderHook(() =>
      useBlockAnimation({ animationEntranceType: 'bounce', animationDirection: 'left' })
    );
    const initial = result.current.motionProps.initial as Record<string, unknown>;
    expect(initial.x).toBe(40);
    expect(initial.scale).toBe(0.8);
