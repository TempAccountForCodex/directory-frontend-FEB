...
describe("useBlockBackground", () => {
  it("none type returns hasBackground=false", () => {
    const { result } = renderHook(() =>
      useBlockBackground({ backgroundType: "none" }),
    );
  ...
  it("solid with missing color returns empty", () => {
    const { result } = renderHook(() =>
      useBlockBackground({ backgroundType: "solid" }),
    );
    expect(result.current.hasBackground).toBe(false);
  });
  ...
  it("gradient to-r produces linear-gradient to right", () => {
    ...
    expect(result.current.backgroundSx.background).toContain(
  ...
  it("image type with missing URL returns empty", () => {
    const { result } = renderHook(() =>
      useBlockBackground({ backgroundType: "image" }),
    );
    expect(result.current.hasBackground).toBe(false);
  });
  ...
  it("image with backgroundParallax=true includes mobile scroll fallback", () => {
    ...
    const mobileSx = result.current.backgroundSx[
  ...
  describe("backgroundOverlays", () => {
    ...
    it("radial overlay background contains radial-gradient with position", () => {
      ...
      const bg = (el.props as { style: { background: string } }).style
        .background;
    ...
    it("linear overlay background contains linear-gradient with angle", () => {
      const { result } = renderHook(() =>
        useBlockBackground({
          ...
          backgroundColor: "#000000",
          backgroundOverlays: [
            { type: "linear", color: "#ff0000", angle: 45, size: "lg" },
          ],
        }),
      );
      const el = result.current.overlayElements[0];
    ...
    it("overlay opacity is normalised from 0-100 to 0-1", () => {
      const { result } = renderHook(() =>
        useBlockBackground({
          ...
          backgroundColor: "#000000",
          backgroundOverlays: [
            { type: "radial", color: "#ff0000", opacity: 60 },
          ],
        }),
      );
      const el = result.current.overlayElements[0];
      const opacity = (el.props as { style: { opacity: number } }).style
        .opacity;
    ...
    it("overlay has pointerEvents none", () => {
      ...
      const el = result.current.overlayElements[0];
      expect(
        (el.props as { style: { pointerEvents: string } }).style.pointerEvents,
      ).toBe("none");
    });
    ...
    it("bottom-left position maps correctly", () => {
      const { result } = renderHook(() =>
        useBlockBackground({
          ...
          backgroundColor: "#000000",
          backgroundOverlays: [
            { type: "radial", color: "#ff0000", position: "bottom-left" },
          ],
        }),
      );
      const el = result.current.overlayElements[0];
    ...
    it("center position maps correctly", () => {
      const { result } = renderHook(() =>
        useBlockBackground({
          ...
          backgroundColor: "#000000",
          backgroundOverlays: [
            { type: "radial", color: "#ff0000", position: "center" },
          ],
        }),
      );
      const el = result.current.overlayElements[0];
    ...
    it("size lg maps to 80%", () => {
      const { result } = renderHook(() =>
        useBlockBackground({
          ...
          backgroundColor: "#000000",
          backgroundOverlays: [
            { type: "radial", color: "#ff0000", size: "lg" },
          ],
        }),
      );
      const el = result.current.overlayElements[0];
