...
describe('TEXT_SHADOW_MAP_SUBHEADING', () => {
  ...
  it('maps subtle to 70% alpha value', () => {
    expect(TEXT_SHADOW_MAP_SUBHEADING.subtle).toBe('0 2px 8px rgba(0,0,0,0.10)');
  });

  it('maps medium to 70% alpha value', () => {
    expect(TEXT_SHADOW_MAP_SUBHEADING.medium).toBe('0 4px 16px rgba(0,0,0,0.17)');
  });

  it('maps dramatic to 70% alpha value', () => {
    expect(TEXT_SHADOW_MAP_SUBHEADING.dramatic).toBe('0 8px 32px rgba(0,0,0,0.25)');
  });
});
...
describe('resolveHeadingTypographySx', () => {
  ...
  it('invalid shadow enum falls back to default (no textShadow emitted)', () => {
    const result = resolveHeadingTypographySx({ headingTextShadow: 'unknown-value' });
  ...
  it('invalid lineHeight enum falls back to normal (1.2)', () => {
    const result = resolveHeadingTypographySx({ headingLineHeight: 'ultra-tight' });
    expect(result.lineHeight).toBe(1.2);
  });
...
describe('resolveSubheadingTypographySx', () => {
  ...
  it('subheading dramatic shadow differs from heading dramatic shadow', () => {
    const heading = resolveHeadingTypographySx({ headingTextShadow: 'dramatic' });
    const subheading = resolveSubheadingTypographySx({ headingTextShadow: 'dramatic' });
  ...
  it('subheading subtle shadow differs from heading subtle shadow', () => {
    ...
    const subheading = resolveSubheadingTypographySx({ headingTextShadow: 'subtle' });
    expect(heading.textShadow).not.toBe(subheading.textShadow);
  });

  it('invalid shadow falls back to default (no textShadow emitted)', () => {
    const result = resolveSubheadingTypographySx({ headingTextShadow: 'super-strong' });
    expect(result.textShadow).toBeUndefined();
  });
});
