...
const DirectoryListingSkeleton: React.FC<{ columns: number }> = React.memo(({ columns }) => {
    ...
    >
      <Box sx={{ textAlign: 'center' }}>
        ...
        <Typography variant="body2" sx={{ color: bodyColor }}>
          {listingsWithCoords.length} location{listingsWithCoords.length !== 1 ? 's' : ''} found
        </Typography>
...
const DirectoryListingBlock: React.FC<DirectoryListingBlockProps> = ({
  ...
  const listingsPerPage = Math.min(Math.max(content.listingsPerPage ?? 12, 1), 24);
  ...
  const emptyMessage =
    content.emptyMessage ?? 'No listings found. Try adjusting your search criteria.';
  ...
  const [selectedCategory, setSelectedCategory] = useState(content.categoryFilter ?? '');
  const [selectedRegion, setSelectedRegion] = useState(content.regionFilter ?? '');
  ...
  }, [currentPage, listingsPerPage, sortBy, debouncedSearch, selectedCategory, selectedRegion]);
  ...
  return (
    <Box sx={{ py: { xs: 4, md: 8 } }}>
      <Container maxWidth="lg">
        ...
        >
          {/* Category chips */}
          {showCategoryFilter && meta.categories && meta.categories.length > 0 && (
            <Box
        ...
        {layout !== 'list' && listings.length > 0 && (
          <Grid container spacing={3}>
            {listings.map((listing: Listing) => (
              <Grid item key={listing.id} xs={gridCols.xs} sm={gridCols.sm} md={gridCols.md}>
                <ListingCard
                  listing={listing}
                  config={cardConfig}
                  colors={cardColors}
