...
const ProductShowcaseBlockBase: React.FC<ProductShowcaseBlockProps> = ({
  ...
  /* --- Resolve products from dynamic data, SSR content, or template items --- */
  const allProducts: Product[] = useMemo(() => {
    if (data?.products) return data.products;
    if (content.products) return content.products;
    if (templateProducts.length > 0) return templateProducts;
    return [];
  }, [data, content.products, templateProducts]);
  ...
  /* --- Client-side filtering (for stock, price, and category) --- */
          ...
          >
            {categoryFilterStyle === "tabs" ? (
              <Tabs
                ...
                scrollButtons="auto"
                textColor="inherit"
                TabIndicatorProps={{ sx: { bgcolor: primaryColor } }}
                sx={{ minHeight: 40 }}
              >
                <Tab
              ...
              <FormControl size="small" sx={{ minWidth: 200 }}>
                <Select
                  ...
                  sx={{ minWidth: 200 }}
                >
                  <MenuItem value="">{allCategoryLabel}</MenuItem>
                  {categories.map((cat) => (
