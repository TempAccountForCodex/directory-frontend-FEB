...
function renderBlock(block: ReturnType<typeof makeBlock>, extraProps: Record<string, any> = {}) {
  return render(
    <MemoryRouter>
      <BlogArticleBlock
        block={block}
...
describe('BlogArticleBlock', () => {
  ...
  it('5. shows info alert when no postIdentifier and no URL params', () => {
    ...
    expect(screen.getByText(/No post identifier provided/i)).toBeInTheDocument();
  });
  ...
  it('8. hides hero image when showImage=false', () => {
    ...
    expect(screen.queryByRole('img', { name: /Understanding React Performance/ })).toBeNull();
  });
  ...
  it('16. renders back button when showBackButton=true', () => {
    setupHookWithData();
    renderBlock(makeBlock({ showBackButton: true, backButtonText: 'Back to Blog' }));
  ...
  it('18. uses custom back button text', () => {
    setupHookWithData();
    renderBlock(makeBlock({ showBackButton: true, backButtonText: 'Go Back to Articles' }));
  ...
  it('23. article body renders description paragraphs', () => {
    setupHookWithData();
    renderBlock(makeBlock());
    expect(screen.getByText('React is a powerful library.')).toBeInTheDocument();
  ...
  it('24. standard layout places TOC in sidebar grid', () => {
    setupHookWithData();
    const { container } = renderBlock(makeBlock({ layout: 'standard', showTableOfContents: true }));
  ...
  it('25. magazine layout renders hero image', () => {
    setupHookWithData();
    const { container } = renderBlock(makeBlock({ layout: 'magazine', showImage: true }));
  ...
  it('30. component is wrapped in React.memo', () => {
    ...
    expect((Comp as any).displayName || (Comp as any).type?.displayName).toBeTruthy();
  });
  ...
  it('35. renders description paragraph as fallback when no headings array', () => {
    ...
    renderBlock(makeBlock());
    expect(screen.getByText('Fallback description text for article.')).toBeInTheDocument();
  });
  ...
  it('36. SEO context setSeoData called with post data when article loads', async () => {
    ...
    render(
      <MemoryRouter>
        <BlogArticleSeoContext.Provider value={{ seoData: null, setSeoData: mockSetSeoData }}>
          <BlogArticleBlock
            block={makeBlock()}
            primaryColor="#378C92"
            headingColor="#252525"
