...
const mockDynamicData = {
  insights: mockPosts,
  pagination: { total: 2, page: 1, limit: 9, totalPages: 1, hasMore: false, hasPrevious: false },
};
...
function setup(
  ...
  } = { data: mockDynamicData, loading: false, error: null, refresh: vi.fn(), lastUpdated: null }
) {
  (useDynamicBlockData as any).mockReturnValue(useDynamicReturnValue);
  mockFetch.mockResolvedValue({
    ok: true,
    json: async () => ({ success: true, categories: ['Technology', 'Business'] }),
  });
  return render(
    <MemoryRouter>
      <BlogFeedBlock {...props} />
...
describe('BlogFeedBlock', () => {
  ...
  it('list layout renders blog cards', () => {
    const block = { ...defaultBlock, content: { ...defaultBlock.content, layout: 'list' } };
    setup({ ...defaultProps, block });
    expect(screen.getByText('First Post')).toBeInTheDocument();
  });
  ...
  it('navigates to default /blog/{slug} when no readMoreLink configured', async () => {
    const block = { ...defaultBlock, content: { ...defaultBlock.content, readMoreLink: '' } };
    setup({ ...defaultProps, block });
    const articles = screen.getAllByRole('article');
    fireEvent.click(articles[0]);
    await waitFor(() => {
  ...
  it('renders category chips when showCategories=true', async () => {
    ...
    await waitFor(() => {
      ...
      const chips = screen
        .getAllByRole('button')
        .filter((el) => el.classList.contains('MuiChip-clickable') || el.closest('.MuiChip-root'));
