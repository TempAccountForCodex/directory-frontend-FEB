...
const now = new Date();
const futureDate = new Date(now.getTime() + 7 * 24 * 60 * 60 * 1000).toISOString(); // 7 days from now
...
describe('EventsListBlock', () => {
  ...
  it('hides search bar when showSearch=false', async () => {
    setupFetchSuccess();
    const block = { ...defaultBlock, content: { ...defaultBlock.content, showSearch: false } };
  ...
  it('shows category filter chips when showFilters=true and categories available', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByRole('group', { name: /category filter/i })).toBeInTheDocument();
    });
  });
  ...
  it('hides pagination when showPagination=false', async () => {
    setupFetchSuccess();
    const block = { ...defaultBlock, content: { ...defaultBlock.content, showPagination: false } };
    renderComponent({ ...defaultProps, block });
    await waitFor(() => {
  ...
  it('calendar layout renders a month grid', async () => {
    ...
    await waitFor(() => {
      ...
      const calendarGrid = container.querySelector('[aria-label="Event calendar"]');
  ...
  it('RSVP button calls POST /api/events/:id/rsvp on click', async () => {
    ...
    mockFetch
      ...
      .mockResolvedValueOnce({ ok: true, json: async () => ({ success: true, rsvpCount: 43 }) }); // RSVP POST

    renderComponent();
    await waitFor(() => {
      expect(screen.getAllByRole('button', { name: /rsvp/i }).length).toBeGreaterThan(0);
    });
  ...
  it('RSVP button becomes disabled after successful RSVP', async () => {
    ...
    mockFetch
      ...
      .mockResolvedValueOnce({ ok: true, json: async () => ({ success: true, rsvpCount: 43 }) });
  ...
  it('RSVP button stores rsvp_{eventId} in sessionStorage', async () => {
    ...
    await waitFor(() => {
      expect(window.sessionStorage.setItem).toHaveBeenCalledWith('rsvp_1', 'true');
    });
  });
  ...
  it('uses emptyMessage from block content for empty state', async () => {
    ...
    const block = {
      ...defaultBlock,
      content: { ...defaultBlock.content, emptyMessage: 'No upcoming events at this time.' },
    };
    renderComponent({ ...defaultProps, block });
    await waitFor(() => {
