...
const StoryPanelBlock: React.FC<StoryPanelBlockProps> = React.memo(
  function StoryPanelBlock({
    content,
    primaryColor = "#1976d2",
    headingColor,
    bodyColor,
    ...
    const stories = useMemo(
      () => (content.stories ?? []).slice(0, 6),
      [content.stories],
    );
    const autoRotate = content.autoRotate !== false;
    const interval = Math.min(
      10000,
      Math.max(3000, content.autoRotateInterval ?? 5000),
    );
    const variant =
      content.variant === "top-bottom" ? "top-bottom" : "side-by-side";
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          {active.subtitle && (
            <Typography
              variant="subtitle1"
              sx={{ color: bodyColor || "text.secondary", mb: 1 }}
            >
              {active.subtitle}
            </Typography>
          )}
