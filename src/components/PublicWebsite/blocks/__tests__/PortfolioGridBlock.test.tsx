...
describe('PortfolioGridBlock', () => {
  ...
  it('renders without crashing with minimal props', () => {
    const block = { id: 1, blockType: 'PORTFOLIO_GRID' as const, sortOrder: 0, content: {} };
  ...
  it('renders description when provided', () => {
    render(<PortfolioGridBlock block={makeBlock()} />);
    expect(screen.getByText('Selected projects we are proud of.')).toBeInTheDocument();
  });
  ...
  it('clicking "All" chip after filtering shows all items again', () => {
    ...
    expect(screen.getAllByText('Brand Refresh').length).toBeGreaterThanOrEqual(1);
    expect(screen.getAllByText('E-Commerce Platform').length).toBeGreaterThanOrEqual(1);
  });
  ...
  it('renders item titles in the grid', () => {
    ...
    expect(screen.getAllByText('Brand Refresh').length).toBeGreaterThanOrEqual(1);
    expect(screen.getAllByText('E-Commerce Platform').length).toBeGreaterThanOrEqual(1);
    expect(screen.getAllByText('Campaign Design').length).toBeGreaterThanOrEqual(1);
  });
  ...
  it('lightbox shows client and year', () => {
    ...
    expect(within(dialog).getAllByText(/Acme Corp/).length).toBeGreaterThanOrEqual(1);
    expect(within(dialog).getAllByText(/2024/).length).toBeGreaterThanOrEqual(1);
  });
  ...
  it('renders with columns=1 without crashing', () => {
    const { container } = render(<PortfolioGridBlock block={makeBlock({ columns: 1 })} />);
    expect(container.firstChild).not.toBeNull();
  });

  // 21
  it('renders with columns=4 without crashing', () => {
    const { container } = render(<PortfolioGridBlock block={makeBlock({ columns: 4 })} />);
    expect(container.firstChild).not.toBeNull();
  });
  ...
  it('renders with only one item without crashing', () => {
    ...
    const { container } = render(<PortfolioGridBlock block={makeBlock({ items: oneItem })} />);
    expect(container.firstChild).not.toBeNull();
  ...
  it('renders with empty content object gracefully', () => {
    const block = { id: 2, blockType: 'PORTFOLIO_GRID' as const, sortOrder: 0, content: {} };
    const { container } = render(<PortfolioGridBlock block={block as any} />);
    expect(container.firstChild).not.toBeNull();
  });
});
