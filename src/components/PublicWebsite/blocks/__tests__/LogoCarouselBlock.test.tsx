...
const baseBlock = {
  ...
  content: {
    ...
    logos: [
      ...
      { imageUrl: 'https://example.com/logo3.png', altText: 'Partner 3', linkUrl: '' },
    ],
...
describe('LogoCarouselBlock', () => {
  ...
  it('renders with grayscale=false without crashing', () => {
    const { container } = render(<LogoCarouselBlock block={noGrayscaleBlock} />);
    expect(container.firstChild).not.toBeNull();
  });
  ...
  it('does not render anchor for logo without linkUrl', () => {
    const noLinkBlock = {
      ...
      content: {
        ...baseBlock.content,
        logos: [
          { imageUrl: 'https://example.com/logo1.png', altText: 'No Link Logo', linkUrl: '' },
        ],
      },
    };
  ...
  it('renders primaryColor prop without crashing', () => {
    const { container } = render(<LogoCarouselBlock block={baseBlock} primaryColor="#ff0000" />);
    expect(container.firstChild).not.toBeNull();
  });
  ...
  it('renders with all default content missing gracefully', () => {
    ...
    const { container } = render(<LogoCarouselBlock block={minimalBlock as any} />);
    expect(container.firstChild).not.toBeNull();
  });
});
