...
describe("ContactBlock", () => {
  ...
  it("split layout renders img when contactImage provided", () => {
    renderBlock({
      layout: "split-image",
      contactImage: "https://example.com/img.jpg",
    });
  ...
  it("imageRatio equal produces 1fr 1fr grid columns", () => {
    ...
    expect(
      container.querySelector('[data-testid="contact-image-cell"]'),
    ).toBeInTheDocument();
    expect(
      container.querySelector('[data-testid="contact-form-cell"]'),
    ).toBeInTheDocument();
  });
  ...
  it("fires onFormSubmit callback on successful submit", async () => {
    ...
    fireEvent.submit(screen.getByLabelText("Name").closest("form")!);
    await waitFor(() =>
      expect(onFormSubmit).toHaveBeenCalledWith("contact", true),
    );
  });
  ...
  it("split-info hides map placeholder when showMap=false", () => {
    ...
    expect(
      screen.queryByTestId("contact-map-placeholder"),
