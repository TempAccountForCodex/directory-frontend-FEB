...
describe('NewsletterBlock', () => {
  ...
  it('renders heading when provided', () => {
    render(<NewsletterBlock block={makeBlock({ heading: 'Get Our Updates' })} />);
    expect(screen.getByText('Get Our Updates')).toBeInTheDocument();
  });

  // 3. Renders description
  it('renders description when provided', () => {
    render(<NewsletterBlock block={makeBlock({ description: 'Our weekly digest.' })} />);
  ...
  it('renders card layout without error', () => {
    expect(() => render(<NewsletterBlock block={makeBlock({ layout: 'card' })} />)).not.toThrow();
  });

  // 8. Inline layout renders form
  it('renders inline layout without error', () => {
    expect(() => render(<NewsletterBlock block={makeBlock({ layout: 'inline' })} />)).not.toThrow();
  });
  ...
  it('does not call fetch when email format is invalid', async () => {
    render(<NewsletterBlock block={makeBlock()} />);
    const inputs = document.querySelectorAll('input[type="email"], input[placeholder*="email"]');
    if (inputs.length > 0) {
  ...
  it('calls DOMPurify.sanitize on description', () => {
    render(<NewsletterBlock block={makeBlock({ description: 'Subscribe to our newsletter.' })} />);
    expect(DOMPurify.sanitize).toHaveBeenCalledWith('Subscribe to our newsletter.');
  });
