...
describe('MenuDisplayBlock', () => {
  ...
  it('shows no menu categories message when categories empty', () => {
    ...
    expect(screen.getByText(/no menu categories configured/i)).toBeInTheDocument();
  });
  ...
  it('cards layout renders items in card containers', () => {
    render(
      <MenuDisplayBlock block={makeBlock({ categories: [sampleCategory], layout: 'cards' })} />
    );
    expect(screen.getByText('Spring Rolls')).toBeInTheDocument();
    expect(screen.getByText('$8.99')).toBeInTheDocument();
  });
  ...
  it('compact layout renders item name and price', () => {
    render(
      <MenuDisplayBlock block={makeBlock({ categories: [sampleCategory], layout: 'compact' })} />
    );
    expect(screen.getByText('Spring Rolls')).toBeInTheDocument();
    expect(screen.getByText('$8.99')).toBeInTheDocument();
  });
  ...
  it('prepends EUR currency symbol (€) to price', () => {
    render(
      <MenuDisplayBlock block={makeBlock({ categories: [sampleCategory], currency: 'EUR' })} />
    );
    expect(screen.getByText('€8.99')).toBeInTheDocument();
  });
  ...
  it('prepends GBP currency symbol (£) to price', () => {
    render(
      <MenuDisplayBlock block={makeBlock({ categories: [sampleCategory], currency: 'GBP' })} />
    );
    expect(screen.getByText('£8.99')).toBeInTheDocument();
  });
  ...
  it('does NOT render dietary icons when showDietaryIcons=false', () => {
    render(
      <MenuDisplayBlock
        block={makeBlock({ categories: [sampleCategory], showDietaryIcons: false })}
      />
    );
  ...
  it('renders multiple categories', () => {
    ...
    render(<MenuDisplayBlock block={makeBlock({ categories: [sampleCategory, mainCourses] })} />);
    expect(screen.getByText('Appetizers')).toBeInTheDocument();
  ...
  it('block renders in DOM even when responsiveHideOnDesktop=true (CSS handles it)', () => {
    const { container } = render(
      <MenuDisplayBlock block={makeBlock({ categories: [], responsiveHideOnDesktop: true })} />
    );
    expect(container.firstChild).toBeTruthy();
  });
  ...
  it('renders gracefully with empty categories array', () => {
    const { container } = render(<MenuDisplayBlock block={makeBlock({ categories: [] })} />);
    expect(container.firstChild).toBeTruthy();
  });
