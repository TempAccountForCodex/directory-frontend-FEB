...
describe('BeforeAfterBlock', () => {
  ...
  it('2. renders without heading gracefully', () => {
    render(<BeforeAfterBlock block={makeBlock({ heading: '' })} primaryColor="#2563eb" />);
    // Should not crash
  ...
  it('6. slider handle is visible (has data-testid or accessible role)', () => {
    const { container } = render(<BeforeAfterBlock block={makeBlock()} primaryColor="#2563eb" />);
  ...
  it('7. multiple pairs render as grid — all before/after labels visible', () => {
    const pairs = [
      makePair({ beforeLabel: 'Before 1', afterLabel: 'After 1', caption: 'Cap 1' }),
      makePair({ beforeLabel: 'Before 2', afterLabel: 'After 2', caption: 'Cap 2' }),
      makePair({ beforeLabel: 'Before 3', afterLabel: 'After 3', caption: 'Cap 3' }),
    ];
    render(<BeforeAfterBlock block={makeBlock({ pairs })} primaryColor="#2563eb" />);
  ...
  it('8. single pair renders without crash', () => {
    render(<BeforeAfterBlock block={makeBlock({ pairs: [makePair()] })} primaryColor="#2563eb" />);
    expect(document.body).toBeTruthy();
  });

  it('9. start position defaults to 50 when not provided', () => {
    const block = makeBlock({ startPosition: undefined });
    const { container } = render(<BeforeAfterBlock block={block} primaryColor="#2563eb" />);
  ...
  it('11. component is React.memo (has displayName)', () => {
    expect(
      BeforeAfterBlock.displayName ?? (BeforeAfterBlock as any).type?.displayName
    ).toBeTruthy();
  });

  it('12. horizontal orientation — container has horizontal data attribute or default', () => {
    const { container } = render(
      <BeforeAfterBlock block={makeBlock({ orientation: 'horizontal' })} primaryColor="#2563eb" />
    );
  ...
  it('13. vertical orientation renders without crash', () => {
    const { container } = render(
      <BeforeAfterBlock block={makeBlock({ orientation: 'vertical' })} primaryColor="#2563eb" />
    );
    expect(container.firstChild).toBeTruthy();
  });
  ...
  it('16. custom sliderColor rendered (no crash with custom color)', () => {
    expect(() =>
      render(
        <BeforeAfterBlock block={makeBlock({ sliderColor: '#ff0000' })} primaryColor="#2563eb" />
      )
    ).not.toThrow();
  });
});
