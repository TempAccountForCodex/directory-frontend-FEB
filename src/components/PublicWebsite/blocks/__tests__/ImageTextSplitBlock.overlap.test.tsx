...
vi.mock('framer-motion', () => ({
  ...
  useTransform: (..._args: any[]) => ({ get: () => '0%', onChange: () => () => {} }),
...
describe('ImageTextSplitBlock — overlap variant', () => {
  ...
  it('default variant renders without motion wrapper', () => {
    render(<ImageTextSplitBlock content={makeContent({ variant: 'default' })} />);
    expect(screen.queryByTestId('motion-div')).toBeNull();
  });

  // 2
  it('overlap variant renders with motion wrapper around image', () => {
    render(<ImageTextSplitBlock content={makeContent({ variant: 'overlap' })} />);
  ...
  it('overlap + imagePosition=left: motion.div wraps image with left slide animation', () => {
    render(
      <ImageTextSplitBlock content={makeContent({ variant: 'overlap', imagePosition: 'left' })} />
    );
  ...
  it('overlap + imagePosition=right: motion.div wraps image with right slide animation', () => {
    render(
      <ImageTextSplitBlock content={makeContent({ variant: 'overlap', imagePosition: 'right' })} />
    );
    const motionDiv = screen.getByTestId('motion-div');
    expect(motionDiv).toBeInTheDocument();
    const initial = JSON.parse(motionDiv.getAttribute('data-initial') || '{}');
  ...
  it('overlap with no image: placeholder is wrapped in motion.div', () => {
    render(<ImageTextSplitBlock content={makeContent({ variant: 'overlap', image: undefined })} />);
    expect(screen.getByTestId('motion-div')).toBeInTheDocument();
  ...
  it('overlap: heading renders correctly', () => {
    render(
      <ImageTextSplitBlock
        content={makeContent({ variant: 'overlap', heading: 'Overlap Heading' })}
      />
    );
  ...
  it('overlap: body renders correctly', () => {
    render(
      <ImageTextSplitBlock
        content={makeContent({ variant: 'overlap', body: '<p>Overlap body text</p>' })}
      />
    );
  ...
  it('overlap: CTA button renders and is present', () => {
    render(
      <ImageTextSplitBlock content={makeContent({ variant: 'overlap', ctaText: 'Get Started' })} />
    );
