...
import { useBlockAnimation, type TextRevealConfig } from '../hooks/useBlockAnimation';
...
const StatusBadge: React.FC<{ badge: StatusBadgeData; align?: 'left' | 'center' }> = memo(
  ({ badge, align = 'left' }) => {
...
export const resolveHeroMinHeight = (value: string | undefined, fallback: string): string =>
  HERO_MIN_HEIGHT[value ?? ''] ?? fallback;
...
const HEADING_EXTRA_SX: Record<string, object> = {
  display: { lineHeight: 0.85, letterSpacing: '-0.04em', textTransform: 'uppercase' as const },
};

export const resolveHeadingFontSizeSx = (value: string | undefined, content?: any): object => {
  if (value === 'custom' && content) {
    ...
    return {
      fontSize: Object.keys(fontSize).length > 0 ? fontSize : HEADING_FONT_SIZE.default,
      ...(content.customLineHeight != null ? { lineHeight: content.customLineHeight } : {}),
      ...(content.customLetterSpacing ? { letterSpacing: content.customLetterSpacing } : {}),
    };
  }
  return {
...
export const resolveSubheadingMaxWidth = (value: string | undefined): string | undefined =>
  value === 'full' ? undefined : (SUBHEADING_MAX_WIDTH[value ?? ''] ?? SUBHEADING_MAX_WIDTH.medium);
                    ...
                    {slide.ctaText && (
                      <Box sx={{ textAlign: 'center' }}>
                        <Button
                          ...
                          onClick={() => onCtaClick(slide.ctaLink || '#', slide.ctaText)}
                          sx={{
                            bgcolor: 'white',
                            color: primaryColor,
                            px: 4,
              ...
              {slides.map((_, index) => (
                <Box
                  ...
                  sx={{
                    ...
                    height: 10,
                    borderRadius: '50%',
                    backgroundColor: index === activeIndex ? primaryColor : 'rgba(255,255,255,0.5)',
                    cursor: 'pointer',
                    transition: prefersReducedMotion.current
                      ? 'none'
                    ...
                    '&:hover': {
                      backgroundColor:
                        index === activeIndex ? primaryColor : 'rgba(255,255,255,0.8)',
                    },
                  }}
                />
              ))}
...
const TextRevealHeading: React.FC<TextRevealHeadingProps> = memo(({ text, config, sx = {} }) => {
  ...
  if (prefersReduced || words.length === 0) {
    return (
      <Typography variant="h2" component="h1" gutterBottom sx={{ fontWeight: 700, ...sx }}>
        {text}
      </Typography>
    );
  }
      ...
      {words.map((word, i) => (
        <span key={i} style={{ overflow: 'hidden', display: 'inline-block', marginRight: '0.3em' }}>
          <motion.span
            ...
            style={{ display: 'inline-block', fontWeight: 700, ...(sx as any) }}
          >
            {word}
          </motion.span>
        </span>
...
const HeroBlock: React.FC<HeroBlockProps> = memo(
  ({
    ...
    const minHeightCentered = resolveHeroMinHeight(content.heroMinHeight, '60vh');
    const minHeightFullBleed = resolveHeroMinHeight(content.heroMinHeight, '80vh');
    const headingFontSizeSx = resolveHeadingFontSizeSx(content.headingFontSize, content);
    const subheadingMaxW = resolveSubheadingMaxWidth(content.subheadingMaxWidth);
    ...
    if (variant === 'full-bleed') {
      ...
      const bgImage = rawBgImage
        ? /^(https?:)?\/\//i.test(rawBgImage) || !/^[a-z][a-z0-9+\-.]*:/i.test(rawBgImage)
      ...
      return (
        <BlockWrapper fields={content}>
          ...
          >
            ...
            />
            <Container maxWidth="lg" sx={{ position: 'relative', zIndex: 1 }}>
              {content.statusBadge && <StatusBadge badge={content.statusBadge} align="center" />}
              {isTextReveal && textRevealConfig ? (
                <TextRevealHeading
                  text={content.heading || 'Welcome'}
                  config={textRevealConfig}
        ...
        <BlockWrapper fields={content}>
          ...
          >
            ...
            >
              ...
              >
                {content.statusBadge && <StatusBadge badge={content.statusBadge} />}
                <Typography
                  variant="h2"
                  component="h1"
                  gutterBottom
    ...
    if (variant === 'carousel') {
      ...
      return (
        <HeroCarousel
          ...
          showIndicators={(content.showSlideIndicators ?? true) && !isSingleSlide}
