...
const makeTemplate = (id: string, overrides: Partial<TemplateSummary> = {}): TemplateSummary => ({
  ...baseTemplate,
  id,
  name: `Template ${id}`,
  ...overrides,
...
describe('TemplateCard', () => {
  ...
  it('renders singular "page" when pageCount is 1', () => {
    render(<TemplateCard template={{ ...baseTemplate, pageCount: 1 }} onClick={onClick} />);
  ...
  it('calls onPreview when Preview button is clicked', () => {
    render(<TemplateCard template={baseTemplate} onClick={onClick} onPreview={onPreview} />);
  ...
  it('renders "Use Template" text in gradient button', () => {
    render(<TemplateCard template={baseTemplate} onClick={onClick} />);
    expect(screen.getByTestId('gradient-btn')).toHaveTextContent('Use Template');
  });
  ...
  it('falls back to raw category value when CATEGORY_LABELS key is missing', () => {
    ...
    const t = { ...baseTemplate, category: 'unknown-cat' as TemplateSummary['category'] };
    render(<TemplateCard template={t} onClick={onClick} />);
...
describe('TemplateGallery', () => {
  ...
  it('shows correct empty state title', () => {
    ...
    expect(screen.getByTestId('empty-title')).toHaveTextContent('No templates found');
  });
  ...
  it('renders the correct number of cards for a larger dataset', () => {
    const templates = Array.from({ length: 6 }, (_, i) => makeTemplate(`id-${i}`));
    render(
      <TemplateGallery
        templates={templates}
        loading={false}
