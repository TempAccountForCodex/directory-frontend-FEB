...
function makeUser(userId: number, userName: string, role: string, color = '#2196f3'): PresenceUser {
...
describe('PresenceIndicator', () => {
  it('renders without crashing with empty user list', () => {
    const { container } = render(<PresenceIndicator users={[]} currentUserId={1} />);
    expect(container).toBeTruthy();
  });

  it('renders avatar for each collaborator', () => {
    const users = [makeUser(1, 'Alice', 'EDITOR'), makeUser(2, 'Bob', 'VIEWER')];
  ...
  it('shows "N editors, M viewers" summary text', () => {
    ...
    expect(screen.getByTestId('presence-summary')).toHaveTextContent(/2 editor/i);
    expect(screen.getByTestId('presence-summary')).toHaveTextContent(/1 viewer/i);
  });

  it('shows overflow indicator when more than 5 users', () => {
    const users = Array.from({ length: 8 }, (_, i) => makeUser(i + 1, `User ${i + 1}`, 'EDITOR'));
    render(<PresenceIndicator users={users} currentUserId={99} />);
  ...
  it('does not show overflow when 5 or fewer users', () => {
    const users = Array.from({ length: 5 }, (_, i) => makeUser(i + 1, `User ${i + 1}`, 'EDITOR'));
    render(<PresenceIndicator users={users} currentUserId={99} />);
  ...
  it('is memoized with React.memo', () => {
    ...
    expect((PresenceIndicator as unknown as { $$typeof: symbol }).$$typeof).toBe(
