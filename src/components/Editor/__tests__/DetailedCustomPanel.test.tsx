...
describe('DetailedCustomPanel', () => {
  ...
  it('shows slug validation error when slugError prop provided', () => {
    render(
      <DetailedCustomPanel {...baseProps} slugError="Slug can only contain lowercase letters" />
    );
    expect(screen.getByText('Slug can only contain lowercase letters')).toBeInTheDocument();
  });
  ...
  it('renders SaveStatus when blockEditorWebsiteId is provided', () => {
    render(<DetailedCustomPanel {...baseProps} blockEditorWebsiteId={5} blockSaveStatus="saved" />);
    expect(screen.getByTestId('save-status')).toBeInTheDocument();
  });

  it('passes blockSaveStatus to SaveStatus component', () => {
    render(
      <DetailedCustomPanel {...baseProps} blockEditorWebsiteId={5} blockSaveStatus="saving" />
    );
