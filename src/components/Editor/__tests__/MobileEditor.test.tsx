...
describe('MobileActionBar', () => {
  ...
  it('calls onPublish when Publish button clicked', () => {
    ...
    withTheme(<MobileActionBar {...defaultActionBarProps} onPublish={onPublish} />);
  ...
  it('calls onPreview when Preview button clicked', () => {
    ...
    withTheme(<MobileActionBar {...defaultActionBarProps} onPreview={onPreview} />);
  ...
  it('action bar has sx display hiding it on md+ breakpoints', () => {
    const { container } = withTheme(<MobileActionBar {...defaultActionBarProps} />);
...
describe('BottomSheet', () => {
  ...
  it('accepts a title prop', () => {
    withTheme(
      <BottomSheet open={true} onClose={vi.fn()} onOpen={vi.fn()} title="Block Library">
        <div>content</div>
      </BottomSheet>
    );
  ...
  it('renders without crashing when no children provided', () => {
    expect(() =>
      withTheme(<BottomSheet open={false} onClose={vi.fn()} onOpen={vi.fn()} />)
    ).not.toThrow();
  });
});
...
describe('ViewportPreviewToolbar', () => {
  ...
  it('calls onViewportChange(375) when mobile preset clicked', () => {
    ...
    withTheme(
      <ViewportPreviewToolbar {...defaultToolbarProps} onViewportChange={onViewportChange} />
    );
  ...
  it('highlights active viewport button (1280 is active by default)', () => {
    withTheme(<ViewportPreviewToolbar {...defaultToolbarProps} viewportWidth={1280} />);
  ...
  it('renders orientation toggle button', () => {
    withTheme(<ViewportPreviewToolbar {...defaultToolbarProps} />);
    expect(screen.getByRole('button', { name: /orientation/i })).toBeInTheDocument();
  });

  it('calls onOrientationToggle when orientation button clicked', () => {
    const onOrientationToggle = vi.fn();
    withTheme(
      <ViewportPreviewToolbar {...defaultToolbarProps} onOrientationToggle={onOrientationToggle} />
    );
  ...
  it('active viewport button has variant=contained (visually highlighted)', () => {
    withTheme(<ViewportPreviewToolbar {...defaultToolbarProps} viewportWidth={375} />);
