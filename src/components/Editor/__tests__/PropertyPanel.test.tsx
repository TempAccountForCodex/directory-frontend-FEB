...
const makeBlock = (overrides?: Record<string, unknown>) => ({
  id: 'block-1',
  blockType: 'HERO',
  content: { heading: 'Welcome', subheading: 'Hello World' } as Record<string, unknown>,
  isVisible: true,
  ...overrides,
});
...
describe('PropertyPanel (Step 9.14.2)', () => {
  ...
  it('does not render panel content when open=false', () => {
    render(<PropertyPanel {...defaultProps} open={false} selectedBlock={makeBlock()} />);

    expect(screen.queryByTestId('property-panel')).toBeNull();
  });

  it('renders panel when open=true with selectedBlock', () => {
    render(<PropertyPanel {...defaultProps} open={true} selectedBlock={makeBlock()} />);
  ...
  it('close button calls onClose', () => {
    const onClose = vi.fn();
    render(
      <PropertyPanel {...defaultProps} open={true} selectedBlock={makeBlock()} onClose={onClose} />
    );
  ...
  it('onChange fires with blockId and partial content on heading change', () => {
    ...
    expect(onChange).toHaveBeenCalledWith('block-1', { heading: 'New Heading' });
  });
  ...
  it('Escape key does NOT call onClose when panel is closed', () => {
    const onClose = vi.fn();
    render(
      <PropertyPanel {...defaultProps} open={false} selectedBlock={makeBlock()} onClose={onClose} />
    );
...
describe('PropertyPanel — Position Controls (Step 9.15.1)', () => {
  ...
  it('shows keyboard shortcut hints as tooltips on move buttons', () => {
    ...
    const moveDownBtn = screen.getByLabelText('Move block down');
    expect(
      moveUpBtn.closest('[title]')?.getAttribute('title') || moveUpBtn.getAttribute('title')
    ).toContain('Ctrl');
    expect(
      moveDownBtn.closest('[title]')?.getAttribute('title') || moveDownBtn.getAttribute('title')
    ).toContain('Ctrl');
  });
  ...
  it('shows alignment section with Left, Center, Right buttons', () => {
    render(
      <PropertyPanel
        {...defaultProps}
        open={true}
        selectedBlock={makeBlock({ content: { heading: 'Hi', alignment: 'center' } })}
      />
    );
  ...
  it('highlights the active alignment button', () => {
    render(
      <PropertyPanel
        {...defaultProps}
        open={true}
        selectedBlock={makeBlock({ content: { heading: 'Hi', alignment: 'left' } })}
      />
    );
  ...
  it('selecting height preset calls onChange with { heightPreset: value }', () => {
    ...
    render(
      <PropertyPanel
        {...defaultProps}
        open={true}
        selectedBlock={makeBlock({ content: { heading: 'Hi', heightPreset: 'auto' } })}
        onChange={onChange}
      />
    );
...
describe('PropertyPanel — Spacing & Layout Presets (Step 9.15.3)', () => {
  ...
  it('spacing dropdown defaults to md when no value in content', () => {
    ...
    const paddingTop = screen.getByLabelText('Padding Top') as HTMLSelectElement;
  ...
  it('changing padding top dropdown calls onChange with { spacingPaddingTop: value }', () => {
    ...
    expect(onChange).toHaveBeenCalledWith('block-1', { spacingPaddingTop: 'lg' });
  });
  ...
  it('changing margin bottom dropdown calls onChange with { spacingMarginBottom: value }', () => {
    ...
    expect(onChange).toHaveBeenCalledWith('block-1', { spacingMarginBottom: 'xl' });
  });
