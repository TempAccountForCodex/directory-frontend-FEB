...
describe('9.11.1 — BottomSheet keyboard', () => {
  ...
  it('closes on Escape key press', () => {
    const onClose = vi.fn();
    withTheme(
      <BottomSheet open={true} onClose={onClose} onOpen={vi.fn()} title="Test Sheet">
        <div>content</div>
      </BottomSheet>
    );
    const dialog = screen.getByRole('dialog');
  ...
  it('Tab navigates through interactive elements within sheet', () => {
    withTheme(
      <BottomSheet open={true} onClose={vi.fn()} onOpen={vi.fn()} title="Tab Nav Test">
...
describe('9.11.1 — ResponsiveEditorLayout keyboard navigation', () => {
  ...
  it('dispatches block-navigate event on ArrowDown', () => {
    ...
    expect((handler.mock.calls[0][0] as CustomEvent).detail.direction).toBe('down');
  ...
  it('dispatches block-navigate event on ArrowUp', () => {
    ...
    expect(handler).toHaveBeenCalledTimes(1);
    expect((handler.mock.calls[0][0] as CustomEvent).detail.direction).toBe('up');
...
describe('9.11.2 — MobileActionBar accessibility', () => {
  ...
  it('has descriptive aria-labels on all buttons', () => {
    ...
    expect(screen.getByRole('button', { name: /preview website/i })).toBeInTheDocument();
  });
...
describe('9.11.2 — ViewportPreviewToolbar accessibility', () => {
  ...
  it('all icon-only buttons have descriptive aria-labels', () => {
    ...
    const orientBtn = screen.getByRole('button', { name: /toggle orientation/i });
