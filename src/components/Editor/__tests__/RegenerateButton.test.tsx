...
vi.mock('../../Dashboard/shared', () => ({
  DashboardIconButton: ({
    ...
    <button data-testid="icon-btn" onClick={onClick} aria-label={ariaLabel || label}>
      {label || 'Regenerate'}
    </button>
  ),
...
describe('RegenerateButton', () => {
  ...
  it('returns null when hasAISessions is false', () => {
    const { container } = render(<RegenerateButton {...defaultProps} hasAISessions={false} />);
    expect(container.innerHTML).toBe('');
  });

  it('returns null for CONTACT block type', () => {
    const { container } = render(<RegenerateButton {...defaultProps} blockType="CONTACT" />);
    expect(container.innerHTML).toBe('');
  });

  it('returns null for IMAGE block type', () => {
    const { container } = render(<RegenerateButton {...defaultProps} blockType="IMAGE" />);
    expect(container.innerHTML).toBe('');
  });
  ...
  it('shows popover confirmation when clicked', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/Regenerate this section\? Current content will be replaced\./)
      ).toBeInTheDocument();
    });
  ...
  it('calls generate-block API on confirm and updates content', async () => {
    ...
    render(<RegenerateButton {...defaultProps} onContentUpdate={mockOnContentUpdate} />);
  ...
  it('closes popover when Cancel is clicked', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByText(/Regenerate this section\?/)).not.toBeInTheDocument();
    });
  });
});
