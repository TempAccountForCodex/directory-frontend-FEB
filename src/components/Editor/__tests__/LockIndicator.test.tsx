...
describe('LockIndicator', () => {
  it('renders nothing when block is not locked', () => {
    const { container } = render(
      <LockIndicator blockId={1} lock={null} currentUserId={10} currentUserRole="EDITOR" />
    );
  ...
  it('shows lock indicator when block is locked by another user', () => {
    ...
    render(<LockIndicator blockId={1} lock={lock} currentUserId={10} currentUserRole="EDITOR" />);
  ...
  it('shows viewer overlay on blocks for VIEWER role', () => {
    render(<LockIndicator blockId={5} lock={null} currentUserId={99} currentUserRole="VIEWER" />);
    expect(screen.getByTestId('viewer-overlay-5')).toBeInTheDocument();
  });

  it('shows "View Only" text for VIEWER role', () => {
    render(<LockIndicator blockId={5} lock={null} currentUserId={99} currentUserRole="VIEWER" />);
    expect(screen.getByTestId('viewer-overlay-5')).toHaveTextContent(/view only/i);
  });

  it('does not show viewer overlay for EDITOR role', () => {
    render(<LockIndicator blockId={5} lock={null} currentUserId={99} currentUserRole="EDITOR" />);
