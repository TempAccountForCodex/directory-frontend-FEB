...
describe('CollaborativeEditorHeader', () => {
  it('renders without crashing', () => {
    const { container } = render(<CollaborativeEditorHeader {...defaultProps} />);
    expect(container).toBeTruthy();
  });

  it('renders the header container', () => {
    render(<CollaborativeEditorHeader {...defaultProps} />);
    expect(screen.getByTestId('collaborative-editor-header')).toBeInTheDocument();
  });

  it('renders PresenceIndicator with active users', () => {
    const users = [
      { userId: 1, userName: 'Alice', userAvatar: null, role: 'EDITOR', color: '#2196f3' },
    ];
  ...
  it('shows "Manage Collaborators" button for OWNER/ADMIN roles', () => {
    render(<CollaborativeEditorHeader {...defaultProps} currentUserRole="ADMIN" />);
  ...
  it('shows disconnected state when not connected', () => {
    ...
    expect(screen.getByTestId('ws-connection-status')).toHaveTextContent(/reconnect/i);
  });

  it('is memoized with React.memo', () => {
    expect(CollaborativeEditorHeader).toHaveProperty('$$typeof');
    expect((CollaborativeEditorHeader as unknown as { $$typeof: symbol }).$$typeof).toBe(
