...
const makeConflictData = (overrides: Partial<ConflictData> = {}): ConflictData => ({
...
describe('ConflictModal', () => {
  ...
  it('renders dialog when open=true', () => {
    ...
    render(<ConflictModal open={true} conflictData={makeConflictData()} onResolve={onResolve} />);
    expect(screen.getByRole('dialog')).toBeInTheDocument();
  });

  it('does not show dialog content when open=false', () => {
    const onResolve = vi.fn();
    render(<ConflictModal open={false} conflictData={makeConflictData()} onResolve={onResolve} />);
  ...
  it('shows conflict warning message', () => {
    render(<ConflictModal open={true} conflictData={makeConflictData()} onResolve={vi.fn()} />);
  ...
  it('shows Keep yours button', () => {
    render(<ConflictModal open={true} conflictData={makeConflictData()} onResolve={vi.fn()} />);
    expect(screen.getByRole('button', { name: /keep yours/i })).toBeInTheDocument();
  });
  ...
  it('shows diff with changed fields side-by-side', () => {
    ...
    render(<ConflictModal open={true} conflictData={conflictData} onResolve={vi.fn()} />);
    fireEvent.click(screen.getByRole('button', { name: /view diff/i }));
  ...
  it('shows inline alert when fetchError prop is set', () => {
    ...
    expect(screen.getByText(/Failed to load server version/)).toBeInTheDocument();
  });
