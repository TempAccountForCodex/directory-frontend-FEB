...
describe('EditorTabs', () => {
  ...
  it('saves selected tab to localStorage on tab change', () => {
    ...
    expect(global.localStorage.setItem).toHaveBeenCalledWith('editor-active-tab', 'simple');
  });

  it('restores previously selected tab from localStorage on mount', () => {
    (global.localStorage.getItem as ReturnType<typeof vi.fn>).mockReturnValue('detailed');
  ...
  it('does not call onChange on mount when localStorage tab matches activeTab prop', () => {
    (global.localStorage.getItem as ReturnType<typeof vi.fn>).mockReturnValue('appearance');
  ...
  it('shows correct active tab based on activeTab prop', () => {
    ...
    expect(screen.getByTestId('tab-layout')).toHaveAttribute('aria-selected', 'true');
    expect(screen.getByTestId('tab-appearance')).toHaveAttribute('aria-selected', 'false');
  });
