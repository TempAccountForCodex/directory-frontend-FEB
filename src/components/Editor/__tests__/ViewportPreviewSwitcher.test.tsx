...
describe('ViewportPreviewSwitcher', () => {
  ...
  it('renders orientation toggle button', () => {
    render(<ViewportPreviewSwitcher {...defaultProps} />);
    const orientationToggle = screen.getByRole('button', { name: /toggle orientation/i });
    expect(orientationToggle).toBeInTheDocument();
  });

  it('calls onWidthChange with correct width when viewport button clicked', () => {
    const onWidthChange = vi.fn();
    render(<ViewportPreviewSwitcher {...defaultProps} onWidthChange={onWidthChange} />);
  ...
  it('calls onOrientationToggle when orientation button clicked', () => {
    ...
    render(<ViewportPreviewSwitcher {...defaultProps} onOrientationToggle={onOrientationToggle} />);

    const orientationBtn = screen.getByRole('button', { name: /toggle orientation/i });
  ...
  it('does not call onWidthChange when same viewport re-selected (exclusive behavior)', () => {
    const onWidthChange = vi.fn();
    render(
      <ViewportPreviewSwitcher {...defaultProps} width={1280} onWidthChange={onWidthChange} />
    );
