...
export interface SaveTemplateModalProps {
  ...
  variant?: string;
  onSaveSuccess: (template: { id: number; name: string; blockType: string }) => void;
}
...
const SaveTemplateModal = React.memo<SaveTemplateModalProps>(function SaveTemplateModal({
  open,
  onClose,
  blockType,
  blockContent,
  ...
  }, [name, description, blockType, variant, blockContent, onSaveSuccess, onClose]);
    ...
    >
      <DialogTitle id="save-template-dialog-title">
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          Save as Template
          <Chip label={blockType} size="small" color="primary" sx={{ fontSize: '0.7rem' }} />
        </Box>
      </DialogTitle>
      <DialogContent>
        {error && (
        ...
        <Typography variant="caption" sx={{ color: 'text.secondary', mt: 1, display: 'block' }}>
          This template will appear in the "My Templates" tab of the block library.
        </Typography>
      </DialogContent>
