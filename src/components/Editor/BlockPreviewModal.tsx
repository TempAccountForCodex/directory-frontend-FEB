...
const BLOCK_TYPE_DEFAULTS: Record<string, Record<string, unknown>> = {
  ...
  FEATURES: {
    ...
    features: [
      { icon: 'star', title: 'Feature 1', description: 'Describe your first feature.' },
      { icon: 'star', title: 'Feature 2', description: 'Describe your second feature.' },
    ],
  },
  TESTIMONIALS: {
    heading: 'What Clients Say',
    testimonials: [{ quote: 'A great experience.', author: 'Alex Johnson', position: 'Founder' }],
  },
  CTA: {
  ...
  CONTACT: { heading: 'Get in Touch', email: 'contact@example.com', showForm: true },
};
...
const BlockPreviewModal = React.memo<BlockPreviewModalProps>(function BlockPreviewModal({
  open,
  onClose,
  block,
  onAddToPage,
  ...
  const handleVariantChange = useCallback((e: { target: { value: string } }) => {
  ...
  const previewBlock = {
    id: 0,
    blockType: block.key,
    content: selectedVariant ? { ...defaultContent, variant: selectedVariant } : defaultContent,
    sortOrder: 0,
  };
  ...
  return (
    <Modal open={open} onClose={onClose} aria-labelledby="block-preview-title">
      <Box
        sx={{
          position: 'absolute',
          top: '50%',
      ...
      >
        ...
        >
          ...
          <IconButton onClick={onClose} aria-label="Close preview" size="small" sx={{ mt: -0.5 }}>
            <CloseIcon />
          </IconButton>
        </Box>
        ...
        >
          ...
          >
            <React.Suspense
              fallback={
                <Box sx={{ p: 2, color: 'text.secondary', fontSize: '0.8rem' }}>
                  Loading preview...
                </Box>
              }
            >
              <BlockRenderer block={previewBlock} viewport="desktop" isPreview />
            </React.Suspense>
          </Box>
        </Box>
              ...
              {block.variants.map((v) => (
                <Box
                  ...
                  sx={{
                    minWidth: 120,
                    border: '1px solid',
                    borderColor: selectedVariant === v ? 'primary.main' : 'divider',
                    borderRadius: 1,
                    overflow: 'hidden',
                    cursor: 'pointer',
                    bgcolor: selectedVariant === v ? 'primary.50' : 'background.default',
                    p: 1,
                  }}
                >
                  <Typography
