...
vi.mock('@mui/icons-material/Add', () => ({ default: () => <span data-testid="icon-add" /> }));
...
describe('RepeaterField — empty state', () => {
  it('renders empty state message when value is empty array', () => {
    ...
    expect(screen.getByText('No items. Click Add to start.')).toBeInTheDocument();
  });
  ...
  it('does NOT render empty state when items exist', () => {
    ...
    expect(screen.queryByText('No items. Click Add to start.')).not.toBeInTheDocument();
  });
...
describe('RepeaterField — disabled prop', () => {
  ...
  it('disables Collapse buttons when disabled=true', () => {
    renderRepeater({ value: [{ _id: 'a' }], disabled: true });
    const collapseBtns = screen.getAllByRole('button', { name: /collapse item|expand item/i });
...
describe('RepeaterField — nested fields rendering', () => {
  ...
  it('renders multiple FieldRenderers when itemSchema has multiple fields', () => {
    ...
    expect(screen.getByTestId('field-renderer-title')).toBeInTheDocument();
    expect(screen.getByTestId('field-renderer-description')).toBeInTheDocument();
  });
  ...
  it('renders fields for each item independently', () => {
    ...
    const inputs = screen.getAllByTestId('field-input-title') as HTMLInputElement[];
  ...
  it('handleFieldChange updates the correct item and field', () => {
    ...
    expect(onChange).toHaveBeenCalledTimes(1);
    const result = onChange.mock.calls[0][0] as Array<{ _id: string; title: string }>;
...
describe('RepeaterField — collapse / expand', () => {
  it('renders collapse button for each item', () => {
    ...
    expect(screen.getByRole('button', { name: /collapse item 1/i })).toBeInTheDocument();
  });

  it('clicking collapse button changes it to expand', () => {
    renderRepeater({ value: [{ _id: 'a' }] });

    const collapseBtn = screen.getByRole('button', { name: /collapse item 1/i });
    fireEvent.click(collapseBtn);

    expect(screen.getByRole('button', { name: /expand item 1/i })).toBeInTheDocument();
    expect(screen.queryByRole('button', { name: /collapse item 1/i })).not.toBeInTheDocument();
  });
  ...
  it('collapses only the clicked item, not others', () => {
    ...
    const item1Collapse = screen.getByRole('button', { name: /collapse item 1/i });
    fireEvent.click(item1Collapse);

    // Item 1 is now "expand" (was collapsed)
    expect(screen.getByRole('button', { name: /expand item 1/i })).toBeInTheDocument();
    // Item 2 is still "collapse" (still expanded)
    expect(screen.getByRole('button', { name: /collapse item 2/i })).toBeInTheDocument();
  });
});
...
describe('RepeaterField — duplicate', () => {
  ...
  it('clicking Duplicate inserts a copy after the item', () => {
    ...
    expect(onChange).toHaveBeenCalledTimes(1);
    const result = onChange.mock.calls[0][0] as Array<{ title: string; _id: string }>;
