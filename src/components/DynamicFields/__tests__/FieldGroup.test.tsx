...
import type { FieldDefinition, FieldGroupDefinition, FieldRendererProps } from '../types';
...
function makeGroup(overrides: Partial<FieldGroupDefinition> = {}): FieldGroupDefinition {
  return {
    id: 'group1',
    label: 'Group One',
    fields: ['field1'],
...
describe('FieldGroup — basic rendering', () => {
  ...
  it('renders a field with no groups defined', () => {
    renderGroup({ fields: [makeField({ name: 'myField' })], values: { myField: 'test' } });
    expect(screen.getByTestId('field-myField')).toBeInTheDocument();
  });

  it('renders field value correctly', () => {
    renderGroup({ fields: [makeField({ name: 'title' })], values: { title: 'Hello World' } });
...
describe('FieldGroup — group sort order', () => {
  it('renders groups in ascending order.order', () => {
    ...
    expect(groupNodes[0]).toHaveAttribute('data-testid', 'field-group-label-gB');
    expect(groupNodes[1]).toHaveAttribute('data-testid', 'field-group-label-gA');
  });
...
describe('FieldGroup — field sort order within group', () => {
  it('renders fields in ascending field.order within a group', () => {
    ...
    const groups: FieldGroupDefinition[] = [
      { id: 'grp', label: 'My Group', order: 1, fields: ['first', 'second', 'last'] },
    ];

    renderGroup({ fields, groups, values: { first: '1', second: '2', last: '3' } });
...
describe('FieldGroup — onChange wiring', () => {
  it('calls onChange with the field name and new value', async () => {
    ...
    const ClickableField: React.FC<FieldRendererProps> = ({ field, onChange: fieldOnChange }) =>
      React.createElement(
        'button',
        {
...
describe('FieldGroup — errors passthrough', () => {
  it('renders an error message for the correct field', () => {
    renderGroup({
      fields: [makeField({ name: 'emailField', label: 'Email', type: FieldType.TEXT })],
      values: { emailField: '' },
...
describe('FieldGroup — empty group handling', () => {
  it('skips rendering a group that has no matching fields', () => {
    const fields: FieldDefinition[] = [makeField({ name: 'f1', group: 'populated' })];
    const groups: FieldGroupDefinition[] = [
