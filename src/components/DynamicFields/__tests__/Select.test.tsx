...
vi.mock('../../Dashboard/shared/DashboardSelect', () => ({
  __esModule: true,
  default: ({ label, value, onChange, disabled, error, multiple, children }: any) => (
...
vi.mock('@mui/material/MenuItem', () => ({
  __esModule: true,
  default: ({ value, children }: any) => <option value={value}>{children}</option>,
}));
...
function renderSelect(props: RenderProps = {}) {
  ...
  return render(
    <Select field={field} value={value} onChange={onChange} disabled={disabled} errors={errors} />
  );
}
...
describe('Select — renders options', () => {
  ...
  it('renders the field label', () => {
    renderSelect({ field: makeField({ options: defaultOptions }, { label: 'Account Status' }) });
...
describe('Select — search filter', () => {
  it('does NOT render search input when searchable is not set', () => {
    renderSelect({ field: makeField({ options: defaultOptions, searchable: false }) });
    expect(screen.queryByTestId('search-input')).not.toBeInTheDocument();
  });

  it('renders search input when searchable=true', () => {
    renderSelect({ field: makeField({ options: defaultOptions, searchable: true }) });
...
describe('Select — error state', () => {
  it('displays error message via FormHelperText when errors prop is provided', () => {
    ...
    expect(screen.getByTestId('helper-text')).toHaveTextContent('This field is required');
  });
