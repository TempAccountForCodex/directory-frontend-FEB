...
describe('NumberInput — step buttons', () => {
  it('calls onChange with incremented value on + button click', async () => {
    ...
    renderNumberInput({ value: 5, onChange, field: makeField({ validation: {} }) });

    const incrementBtn = screen.getByRole('button', { name: 'Increment value' });
  ...
  it('calls onChange with decremented value on - button click', async () => {
    ...
    const decrementBtn = screen.getByRole('button', { name: 'Decrement value' });
  ...
  it('uses custom step when field.ui.props.step is set', async () => {
    ...
    await userEvent.click(screen.getByRole('button', { name: 'Increment value' }));
...
describe('NumberInput — step button disabled states', () => {
  ...
  it('increment button is NOT disabled when value < max', () => {
    ...
    expect(screen.getByRole('button', { name: 'Increment value' })).not.toBeDisabled();
  });
