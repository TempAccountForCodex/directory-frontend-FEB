...
function makeField(type: FieldType, overrides = {}): FieldRendererProps['field'] {
  return {
    name: 'test',
    label: 'Test',
    type,
...
describe('FallbackField', () => {
  it('renders unsupported field type message', () => {
    ...
    expect(screen.getByText(/Unsupported field type: REPEATER/i)).toBeInTheDocument();
  });
...
describe('registerFieldComponent', () => {
  ...
  it('overwrites an existing registration', () => {
    const OtherMock: React.FC<FieldRendererProps> = () => React.createElement('div', null, 'other');
    registerFieldComponent(FieldType.TEXT, MockComponent);
  ...
  it('allows registering all FieldType values', () => {
    const MockFieldComp: React.FC<FieldRendererProps> = () => React.createElement('div', null);
...
describe('registerFieldComponent — PROD QA null/undefined guard', () => {
  ...
  it('does not throw when registering a valid component', () => {
    const ValidComp: React.FC<FieldRendererProps> = () => React.createElement('div', null);
    expect(() => {
