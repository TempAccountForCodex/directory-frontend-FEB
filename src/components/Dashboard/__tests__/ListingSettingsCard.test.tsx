...
describe('ListingSettingsCard', () => {
  ...
  it('toggle ON triggers extract and displays completeness', async () => {
    ...
    mockedAxios.get.mockResolvedValueOnce({
      data: { success: true, data: { score: 75, missing: ['phone', 'email'], suggestions: [] } },
    });
  ...
  it('toggle OFF shows confirmation dialog', async () => {
    mockedAxios.get.mockResolvedValueOnce({
      data: { success: true, data: { score: 80, missing: [], suggestions: [] } },
    });
  ...
  it('shows AI generations remaining for paid plan when opted in', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/8 of 10 AI generations remaining/i)).toBeInTheDocument();
    });
  });
