...
vi.mock('../shared', () => ({
  ...
  ConfirmationDialog: ({ open, onConfirm, onCancel, title, confirmLabel, loading }: any) =>
    open ? (
      <div data-testid="confirm-dialog">
        <p>{title}</p>
        <button data-testid="confirm-action" onClick={onConfirm} disabled={loading}>
          {confirmLabel}
        </button>
...
describe('ListingEditTab', () => {
  ...
  beforeEach(() => {
    ...
    mockedAxios.get.mockResolvedValue({
      data: { success: true, data: { score: 80, missing: ['phone'], suggestions: [] } },
    });
  });
  ...
  it('renders status badge as Published when isPublic is true', async () => {
    render(
      <ListingEditTab {...defaultProps} websiteData={{ ...baseWebsiteData, isPublic: true }} />
    );
    await waitFor(() => {
      const badge = screen.getByTestId('status-badge');
  ...
  it('renders form fields with pre-populated data', async () => {
    ...
    await waitFor(() => {
      const nameInput = screen.getByTestId('input-business-name').querySelector('input')!;
  ...
  it('enforces max 10 tags', async () => {
    ...
    render(
      <ListingEditTab {...defaultProps} websiteData={{ ...baseWebsiteData, tags: tenTags }} />
    );
  ...
  it('blocks publish when completeness < 60%', async () => {
    mockedAxios.get.mockResolvedValue({
      data: {
        success: true,
        data: { score: 45, missing: ['phone', 'email', 'address'], suggestions: [] },
      },
    });
    ...
    await waitFor(() => {
      expect(screen.getByText(/completeness must be at least 60%/i)).toBeInTheDocument();
    });
  });
  ...
  it('renders character counter for shortDescription', async () => {
    ...
    await waitFor(() => {
      ...
      expect(counter).toHaveTextContent(`${baseWebsiteData.shortDescription.length}/500`);
    });
  });
  ...
  it('calls AI enhance endpoint', async () => {
    mockedAxios.post.mockResolvedValueOnce({
      data: { success: true, data: { shortDescription: 'Enhanced desc', tags: ['ai'] } },
    });
