...
describe('MarqueeBlock', () => {
  ...
  it('renders text items with separator', () => {
    const block = makeBlock({ items: ['Alpha', 'Beta', 'Gamma'], separator: '•' });
  ...
  it('pauses animation on mouseEnter when pauseOnHover=true', () => {
    ...
    const { container } = render(<MarqueeBlock block={block} />);
    const scrollEl = container.querySelector('[data-testid="marquee-track"]') as HTMLElement;
  ...
  it('BLOCK_TYPES.MARQUEE has expected shape (static verification)', () => {
    ...
    const mockMarquee = {
      ...
      sortOrder: 185,
      category: 'content',
      capabilities: { isDynamic: false, supportsBackground: true, supportsVisibility: true },
      defaults: {
        speed: 'medium',
        direction: 'left',
        pauseOnHover: true,
