...
function CardContact({ data, theme }: Omit<ContactBlockProps, "variant">) {
  ...
  return (
    <Box sx={{ bgcolor: theme.bgSecondary, py: { xs: 8, md: 12 }, px: 3 }}>
      ...
      >
        <Box sx={{ bgcolor: theme.primaryColor, p: 5, display: "flex", flexDirection: "column", justifyContent: "center" }}>
          <FadeIn direction="left">
            ...
            {contact.phone && (
              <Box sx={{ display: "flex", alignItems: "center", gap: 2, mb: 3 }}>
                ...
                <Typography sx={{ color: "#fff", fontFamily: theme.fontFamily }}>{contact.phone}</Typography>
              </Box>
            )}
            {contact.email && (
              <Box sx={{ display: "flex", alignItems: "center", gap: 2, mb: 3 }}>
                <EmailIcon sx={{ color: "rgba(255,255,255,0.7)" }} />
                <Typography sx={{ color: "#fff", fontFamily: theme.fontFamily }}>{contact.email}</Typography>
              </Box>
            )}
            {contact.address && (
              <Box sx={{ display: "flex", alignItems: "flex-start", gap: 2 }}>
                <LocationOnIcon sx={{ color: "rgba(255,255,255,0.7)", mt: 0.3 }} />
                <Typography sx={{ color: "#fff", fontFamily: theme.fontFamily }}>{contact.address}</Typography>
              </Box>
            )}
          </FadeIn>
        </Box>
    ...
    <Box sx={{ bgcolor: theme.bgPrimary, py: { xs: 8, md: 12 }, px: 3 }}>
      <Box sx={{ maxWidth: 640, mx: "auto" }}>
        <FadeIn>
          <Typography
            variant="h3"
            sx={{ fontFamily: theme.fontFamily, fontWeight: 800, color: theme.headingColor, textAlign: "center", mb: 1 }}
          >
            Contact Us
          </Typography>
          <Box
            sx={{ width: 48, height: 3, bgcolor: theme.accentColor, mx: "auto", borderRadius: 999, mb: 5 }}
          />
        </FadeIn>
        {submitted ? (
          <Alert severity="success">Thank you! We'll be in touch soon.</Alert>
              ...
              <Grid container spacing={2.5}>
                ...
                <Grid item xs={12}>
                  <TextField
                    ...
                    onChange={handleChange} required multiline rows={4} sx={inputSx}
                  />
                </Grid>
